...
describe("value instrument language", () => {
  ...
  it("keeps Points, Keys, and tickets from pretending to be money", () => {
    ...
    expect(VALUE_INSTRUMENTS["promoshare-tickets"].isNot).toMatch(/not a guaranteed prize/i);
    expect(VALUE_INSTRUMENTS["promoshare-tickets"].is).toMatch(/named draw/i);
    expect(VALUE_STORY.ticketsChance).toMatch(/not a prize/i);
  });
  ...
  it("explains Save & Win as a no-loss pot of parked Gems", () => {
    const pot = getValueInstrument("save-and-win");
    expect(pot.is).toMatch(/no-loss/i);
    expect(pot.is).toMatch(/100%/);
    expect(pot.isNot).toMatch(/not investing/i);
    expect(VALUE_STORY.saveAndWin).toMatch(/Park Gems/i);
  });

  it("walks the layers in the story people need to hear", () => {
    const layers = getValueInstrumentsByLayer();
    expect(layers.map((layer) => layer.id)).toEqual(["everyday", "value", "access", "chances", "parked"]);
    ...
    expect(layers[3].instruments.map((item) => item.id)).toEqual(["promoshare-tickets"]);
    expect(layers[4].instruments.map((item) => item.id)).toEqual(["save-and-win"]);
