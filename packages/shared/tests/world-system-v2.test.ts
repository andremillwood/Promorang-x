...
} from "../src/world-system-v2";

const day = (offset: number) => new Date(Date.now() - offset * 24 * 60 * 60 * 1000).toISOString();

describe("feature flags", () => {
  it("keeps foundation on and later phases off by default", () => {
  ...
  it("can hide identity and unlock competition only via env", () => {
...
describe("resonance", () => {
  it("gives nothing for login, tap, or scroll", () => {
  ...
  it("forms Resonance, then reveals a House only at the threshold", () => {
  ...
  it("does not double-count the same action id", () => {
...
describe("house assignment", () => {
  it("reveals once and refuses oscillation under the switch lead", () => {
...
describe("influence and reputation", () => {
  it("counts each verified row once and never treats the score as money", () => {
  ...
  it("hides Reputation until three fulfilled moves and ignores inactivity", () => {
...
describe("return chains", () => {
  it("does not label a lone share as a returned Throw", () => {
  ...
  it("attributes downstream verified movement to the origin actor", () => {
    const chain = resolveReturnChain({
      origin: { id: "t1", userId: "andre", actionType: "share_completed" },
      downstream: [
...
describe("traits and titles", () => {
  it("earns Reliable and Explorer from real history only", () => {
  ...
  it("does not invent titles without criteria", () => {
...
describe("techniques, challenges, place influence, secrets", () => {
  it("blocks Techniques until the phase and House match", () => {
  ...
  it("does not score Challenges while competition is flagged off", () => {
  ...
  it("hides Place House share until enough real evidence and applies decay", () => {
    const hidden = resolvePlaceHouseShare(
      Array.from({ length: PLACE_INFLUENCE_MIN - 1 }, (_, index) => ({
        id: `p${index}`,
        houseKey: "grove" as const,
        verifiedAt: day(1),
      })),
    );
    ...
    const visible = resolvePlaceHouseShare([
      ...Array.from({ length: 4 }, (_, index) => ({ id: `g${index}`, houseKey: "grove" as const, verifiedAt: day(1) })),
  ...
  it("never hides a Secret after money or Gems were committed", () => {
  ...
  it("keeps elemental modifiers small", () => {
    expect(resolveElementalModifier("fire", "water")).toBe(0.05);
    expect(resolveElementalModifier("fire", "fire")).toBe(0);
  });

  it("does not score a Convergence until competition is on, and stays quiet with no activity", () => {
  ...
  it("never treats a Rumour as factual until confirmed", () => {
  ...
  it("refuses a second issuance of the same Artifact key", () => {
...
describe("identity card", () => {
  it("does not put House on the card before reveal when identity is on", () => {
