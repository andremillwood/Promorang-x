...
export function resolveMomentOccurrence(
  moment: RecurringMomentLike,
  referenceDate = new Date(),
): ResolvedMomentOccurrence {
  ...
  const originalWall = new Date(
    Date.UTC(
      originalWallParts.year,
      originalWallParts.month - 1,
      originalWallParts.day,
      originalWallParts.hour,
      originalWallParts.minute,
      originalWallParts.second,
      originalStart.getUTCMilliseconds(),
    ),
  );
  let candidate = new Date(originalStart);
  let occurrenceNumber = 1;
  const weekdays = (moment.recurrence_by_weekday?.length ? moment.recurrence_by_weekday : [originalWall.getUTCDay()])
    .map(Number)
  ...
  for (let elapsedDays = 1; elapsedDays <= maxDays; elapsedDays += 1) {
    ...
    const monthDifference =
      (candidateWall.getUTCFullYear() - originalWall.getUTCFullYear()) * 12 +
      candidateWall.getUTCMonth() -
      originalWall.getUTCMonth();
      ...
      moment.recurrence_frequency === "daily"
        ...
        : moment.recurrence_frequency === "monthly"
          ? monthDifference > 0 &&
            ...
            candidateWall.getUTCDate() === Math.min(requestedMonthDay, lastDay)
  ...
  const available =
    occurrenceNumber <= countLimit && candidate.getTime() <= untilTime && candidate.getTime() >= referenceDate.getTime();
