...
const ROLE_LENSES: Record<Exclude<StakeholderNavRole, "admin">, StakeholderLens> = {
  ...
  agency: lensParts("agency", {
    ...
    promise: "Connect a client, activate under that client, then prove the first recorded result.",
    ticker: "Prove one client loop end to end",
    putIn: {
      label: "Connect client",
      href: "/dashboard?view=studio&tab=clients",
      detail: "Choose the brand or venue account first so ownership, activity, and proof stay attached to the right client.",
      stub: "CLIENT",
    },
    promoCardMeaning: "The member-facing proof that a client benefit reached a real person.",
    worldLabel: "World",
    worldHref: "/discover",
    worldMeaning: "Where the client’s benefit should show up.",
    activityLabel: "Results",
    activityHref: "/dashboard?view=studio&tab=impact",
    activityMeaning: "Recorded client outcomes ready to review and package.",
    extras: [
      { label: "Activations", href: "/dashboard?view=studio&tab=activations", group: "manage" },
      ...PEOPLE_EXTRAS,
    ],
  }),
