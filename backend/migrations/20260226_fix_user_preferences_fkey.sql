-- Migration: Fix user_preferences foreign key constraint
-- The FK "user_preferences_user_id_fkey" may reference the wrong table,
-- causing inserts to fail during onboarding. This drops the constraint
-- and recreates it pointing to auth.users(id).

-- 1. Drop the existing broken FK (if it exists)
ALTER TABLE public.user_preferences
  DROP CONSTRAINT IF EXISTS user_preferences_user_id_fkey;

-- 2. Recreate the FK correctly referencing auth.users(id)
ALTER TABLE public.user_preferences
  ADD CONSTRAINT user_preferences_user_id_fkey
  FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE;

-- 3. Create a SECURITY DEFINER function to upsert preferences,
--    bypassing RLS so profile creation always succeeds.
