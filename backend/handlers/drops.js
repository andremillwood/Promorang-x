...
// Get drops created by current user - MUST be before /:id route
    ...
    if (!supabase) {
      ...
      return res.status(201).json({
        ...
        drop: {
          ...
          creator_name: req.user.display_name || req.user.username || 'User',
          title,
          description,
          drop_type,
...
// Submit completed work for a drop application
router.post('/:id/submit', async (req, res) => {
  try {
    ...
    // Find the user's application for this drop
    ...
    if (findError || !application) {
      return res.status(404).json({
        success: false,
        error: 'No active application found for this drop'
      });
    }
    ...
    if (updateError) {
      console.error('Database error submitting work:', updateError);
...
// Start work on a drop (transition from pending to in_progress)
router.post('/:id/start', async (req, res) => {
  try {
    ...
    if (findError || !application) {
      return res.status(404).json({
        success: false,
        error: 'No pending application found for this drop'
      });
    }
    ...
    if (updateError) {
      console.error('Database error starting work:', updateError);
...
// Approve/reject application with gem distribution
router.post('/:dropId/applications/:applicationId', async (req, res) => {
  try {
    ...
    if (!supabase) {
      return res.json({
        success: true,
        gems_awarded: action === 'approve' ? 25 : 0,
        message: `Application ${action}d successfully`
      });
    }

    // Get the drop details for gem reward
    const { data: drop, error: dropError } = await supabase
      .from('drops')
      .select('*')
      .eq('id', dropId)
    ...
    if (action === 'approve') {
      ...
      gemsAwarded = drop.gem_reward_base || 0;
      updateData.gems_earned = gemsAwarded;

      // Award gems to the user if there are gems to award
      if (gemsAwarded > 0 && drop.gem_pool_remaining >= gemsAwarded) {
        // Update user's gem balance
        const { error: userUpdateError } = await incrementGems(existingApp.user_id, gemsAwarded);

        if (userUpdateError) {
          console.error('Error awarding gems via helper:', userUpdateError);
        }

        // Reduce drop's remaining gem pool
        await supabase
          .from('drops')
          .update({
            gem_pool_remaining: drop.gem_pool_remaining - gemsAwarded,
            updated_at: new Date().toISOString()
          })
          .eq('id', dropId);

        invalidateCache(`drops:item:${dropId}`);
        invalidateCache('drops:list');
      }
    }
    ...
    res.json({
      ...
      message: `Application ${action}d successfully${gemsAwarded > 0 ? ` - ${gemsAwarded} gems awarded` : ''}`
    });
  } catch (error) {
