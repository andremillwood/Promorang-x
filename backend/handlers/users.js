...
const { authMiddleware, generateToken, decodeToken } = require('../lib/auth');
...
router.put('/:id', async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, error: 'Internal server error while updating user' });
  }
});
...
router.post('/become-advertiser', async (req, res) => {
  try {
    const { brand_name: brandName, brand_logo_url: brandLogoUrl } = req.body;

    const advertiserDetails = {
      user_type: 'advertiser',
      advertiser_tier: 'free',
      display_name: brandName || req.user.display_name,
      avatar_url: brandLogoUrl || req.user.avatar_url,
      updated_at: new Date().toISOString()
    };
...
// Register Instagram username and generate verification code
router.post('/register-instagram', async (req, res) => {
  try {
    const { instagram_username: instagramUsername } = req.body;
    if (!instagramUsername) {
      return res.status(400).json({ success: false, error: 'Instagram username is required' });
    }

    const verificationCode = `promo-${Math.random().toString(36).substring(2, 8)}`;
    ...
    if (supabase) {
      ...
      if (error) {
        ...
        return res.status(500).json({ success: false, error: 'Failed to register Instagram' });
      }
    }
    ...
    res.json({
      ...
      message: 'Instagram registered. Please place the code in your bio.'
    });
  } catch (error) {
    console.error('Error in register-instagram:', error);
    res.status(500).json({ success: false, error: 'Internal server error' });
  }
});

// Claim Instagram points (Verification)
router.post('/claim-instagram-points', async (req, res) => {
  try {
    if (!supabase) {
      return res.json({ success: true, message: 'Points claimed (mock)' });
    }
    ...
    // In a real production app, we would use Firecrawl/Scraper here to check the bio
    // For now, we simulate success if they have a code registered
    if (!user.instagram_verification_code) {
      return res.status(400).json({ success: false, error: 'No verification pending' });
    }

    const tierPoints = { super: 1000, premium: 750, free: 500 };
    const pointsToAward = tierPoints[user.user_tier || 'free'] || 500;

    const { error: updateError } = await supabase
      .from('users')
      .update({
        instagram_verified: true,
        points_balance: (user.points_balance || 0) + pointsToAward,
        instagram_verification_code: null, // Clear after use
        updated_at: new Date().toISOString()
      })
      .eq('id', req.user.id);

    if (updateError) {
      console.error('Error updating verification points:', updateError);
    ...
    res.json({
      ...
      message: `Successfully verified! ${pointsToAward} points awarded.`
    });
  } catch (error) {
    console.error('Error in claim-instagram-points:', error);
    res.status(500).json({ success: false, error: 'Internal server error' });
  }
});
