...
const { mapActionToPathDimension } = require('./worldLayer');

const WORLD_SYSTEM_RULE_VERSION = '2026-09-08.v2a';

function resolveWorldSystemPhases(env) {
  if (env === undefined) {
...
const WORLD_HOUSE_KEYS = Object.keys(WORLD_HOUSES);

const ELEMENT_TO_HOUSE = {
  fire: 'ember',
  water: 'tide',
  air: 'radiant',
  earth: 'grove',
...
function resolveHouse(key) {
  if (!key) return null;
  return WORLD_HOUSES[key] || null;
}

const IGNORED_RESONANCE_ACTIONS = new Set([
  'login',
  'open_screen',
  'tap',
  'scroll',
 ...
 */
const RESONANCE_ACTION_WEIGHTS = {
  CUSTOM: { fire: 1 },
...
const RESONANCE_STAGE_TOTALS = { forming: 3, affinity: 6, reveal: 10 };
const HOUSE_REVEAL_SHARE = 0.4;
const HOUSE_REVEAL_LEAD = 2;
const HOUSE_SWITCH_LEAD = 4;

function emptyResonanceScores() {
  return { fire: 0, water: 0, air: 0, earth: 0 };
}

function mapActionToResonance(actionType) {
...
function rankedElements(scores) {
  return Object.entries(scores).sort((a, b) => b[1] - a[1]);
}

function resolveResonance(actions) {
...
function resolveHouseAssignment(input) {
  ...
  if (!candidate || !input.revealEligible) {
    return { houseKey: (current && current.key) || null, persist: false, revealed: false, switched: false };
  }
  if (!current) {
...
const INFLUENCE_ACTION_WEIGHTS = {
...
function resolveInfluence(actions) {
...
function resolveReputation(actions) {
  let fulfilled = 0;
  let invalid = 0;
  for (const action of actions || []) {
    if (action.invalid) {
...
function isThrowAction(actionType) {
  if (!actionType) return false;
  return THROW_ACTION_TYPES.includes(actionType);
}

function resolveReturnChain(input) {
  ...
  const influence = hops.reduce((sum, action) => {
    const weight = action.actionType ? (INFLUENCE_ACTION_WEIGHTS[action.actionType] && INFLUENCE_ACTION_WEIGHTS[action.actionType].weight) || 0 : 0;
    return sum + weight;
  }, 0);
...
function resolveTraits(actions) {
  const list = actions || [];
  const earned = [];
...
function resolveTechniquePermission(input) {
...
function resolveElementalModifier(attacker, defender) {
...
function scoreChallenge(input) {
  ...
  for (const action of input.actions || []) {
    ...
    if (seen.has(key)) continue;
    seen.add(key);
    score += (INFLUENCE_ACTION_WEIGHTS[action.actionType] && INFLUENCE_ACTION_WEIGHTS[action.actionType].weight) || 1;
  }
...
function decayWeight(verifiedAt, now) {
...
function resolvePlaceActivity(count) {
...
function resolvePlaceHouseShare(actions, now) {
  if (now === undefined) now = new Date();
  const totals = { ember: 0, tide: 0, radiant: 0, grove: 0 };
  let weight = 0;
  for (const action of actions || []) {
...
function resolveRumourLifecycle(input) {
...
function resolveSecretReveal(input) {
...
function resolveExtendedRunRole(key) {
  if (!key) return null;
  return EXTENDED_CREW_RUN_ROLES[key] || null;
}

function resolveIdentityCard(input) {
  const phases = input.phases || resolveWorldSystemPhases();
  if (!phases.identity) {
    return {
      house: null,
  ...
  const house = input.assignment && input.assignment.houseKey
  ...
  return {
    ...
    influenceLine: (input.influence && input.influence.line) || null,
    reputationLine: input.reputation && input.reputation.visible ? input.reputation.line : null,
    throwLine: input.returnChain && input.returnChain.counted ? input.returnChain.line : null,
  };
}
...
module.exports = {
  ...
  FACTION_TO_HOUSE,
  resolveHouse,
  IGNORED_RESONANCE_ACTIONS,
  RESONANCE_ACTION_WEIGHTS,
  RESONANCE_STAGE_TOTALS,
  HOUSE_REVEAL_SHARE,
  HOUSE_REVEAL_LEAD,
  HOUSE_SWITCH_LEAD,
  emptyResonanceScores,
  mapActionToResonance,
  resolveResonance,
  resolveHouseAssignment,
  INFLUENCE_ACTION_WEIGHTS,
  emptyInfluenceKinds,
  resolveInfluence,
  resolveReputation,
  THROW_ACTION_TYPES,
