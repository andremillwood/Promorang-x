-- COMPLETE PIECES TRADING MIGRATION
-- Full system with dividends, governance, oracles, and revenue distribution
-- Run this entire file in Supabase SQL Editor

-- =====================================================
-- STEP 1: DROP EXISTING POLICIES (to avoid 42710 errors)
-- =====================================================
...
-- Only drop the dividend functions, NOT touch_updated_at (it has dependents)
DROP FUNCTION IF EXISTS public.calculate_piece_dividends(public.piece_type, uuid, timestamptz, timestamptz);
DROP FUNCTION IF EXISTS public.calculate_piece_dividends(text, uuid, timestamptz, timestamptz);
DROP FUNCTION IF EXISTS public.calculate_and_distribute_dividends;
-- NOTE: We use CREATE OR REPLACE for touch_updated_at below instead of DROP

-- =====================================================
-- STEP 3: CREATE TABLES
-- =====================================================

-- 3.1 PIECE FEE STRUCTURES
...
INSERT INTO public.piece_fee_structures (piece_type, fee_type, fee_percent) VALUES
  ...
  ('venue', 'platform', 0.0150),
  ('venue', 'creator', 0.0100),
  ('venue', 'liquidity', 0.0050),
  ('host', 'platform', 0.0100),
  ('host', 'creator', 0.0050),
  ('host', 'liquidity', 0.0050),
  ('moment', 'platform', 0.0100),
  ('moment', 'creator', 0.0050),
  ('moment', 'liquidity', 0.0050)
ON CONFLICT DO NOTHING;

-- 3.2 PIECE REVENUE SOURCES
...
-- 3.5 PIECE GOVERNANCE PROPOSALS
...
CREATE INDEX IF NOT EXISTS idx_piece_oracles_asset ON public.piece_price_oracles(piece_type, asset_id);

-- =====================================================
-- STEP 4: CREATE FUNCTIONS
-- =====================================================

-- 4.1 TOUCH UPDATED AT FUNCTION
...
-- 4.2 CALCULATE PIECE DIVIDENDS FUNCTION
...
-- =====================================================
-- STEP 8: UPDATE ENUM TYPE (if needed)
-- =====================================================
