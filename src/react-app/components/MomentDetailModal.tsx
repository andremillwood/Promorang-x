      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-950/80 backdrop-blur-md overflow-y-auto">
        <div className="relative w-full max-w-2xl bg-white rounded-3xl shadow-2xl border border-gray-100 overflow-hidden my-8 animate-in fade-in zoom-in-95 duration-200">
          ...
          <div className="p-6 md:p-8 max-h-[65vh] overflow-y-auto space-y-6">
            {/* Overview */}
            <div>
              <h3 className="text-xs font-black uppercase tracking-wider text-gray-400 mb-2">Moment Overview</h3>
            ...
            <div className="p-4 bg-orange-50 rounded-2xl border border-orange-200/60 flex items-center justify-between">
              <div className="flex items-center space-x-3">
                <div className="p-2.5 bg-orange-500 text-white rounded-xl shadow-md shadow-orange-500/20">
                  <Award className="w-5 h-5" />
                </div>
                <div>
                  <p className="text-xs text-orange-950 font-black">Total Activation Earning Potential</p>
                  <p className="text-[11px] text-orange-800">Complete all sub-moments & missions</p>
                </div>
              </div>
              <div className="text-right">
                <span className="text-xl font-black text-orange-600">+{totalSubPoints}</span>
                <span className="text-xs font-bold text-orange-900 ml-1">Points</span>
              </div>
            </div>
              ...
              <div className="space-y-3">
                {subMoments.map((sub, idx) => {
                  ...
                  return (
                    <div
                      key={sub.id}
                      className="p-4 bg-gray-50 hover:bg-purple-50/50 border border-gray-200/80 rounded-2xl transition-all flex flex-col md:flex-row md:items-center justify-between gap-3"
                    >
                      <div className="flex items-start space-x-3">
                        <span className="w-6 h-6 rounded-full bg-purple-100 text-purple-700 text-xs font-black flex items-center justify-center shrink-0 mt-0.5">
                          {idx + 1}
                        </span>
                        <div>
                          <div className="flex items-center space-x-2">
                      ...
                      <div className="flex items-center justify-between md:justify-end space-x-3 shrink-0 pt-2 md:pt-0 border-t md:border-t-0 border-gray-100">
                        <span className="text-xs font-black text-purple-700 bg-purple-100/70 px-2.5 py-1 rounded-lg">
                          +{sub.points} pts
                        </span>
                        {mStatus === 'COMPLETED' ? (
                          <button
                            onClick={() => setSelectedMission(sub)}
                            className="px-3 py-1.5 bg-emerald-600/15 text-emerald-700 border border-emerald-500/30 text-xs font-bold rounded-xl flex items-center space-x-1"
                          >
                          ...
                          <button
                            onClick={() => setSelectedMission(sub)}
                            className="px-3 py-1.5 bg-amber-500/15 text-amber-700 border border-amber-500/30 text-xs font-bold rounded-xl flex items-center space-x-1"
                          >
                          ...
                          <button
                            onClick={() => setSelectedMission(sub)}
                            className="px-3 py-1.5 bg-gray-900 hover:bg-purple-600 text-white text-xs font-bold rounded-xl transition-colors shadow-sm"
                          >
                            Start Mission
                          </button>
                        )}
