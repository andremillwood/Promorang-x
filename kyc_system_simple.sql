...
-- Simplified version avoiding custom types in function signatures
...
-- =====================================================
-- 3. FUNCTIONS (Simple version - no custom types in params/returns)
-- =====================================================

DROP FUNCTION IF EXISTS public.get_user_kyc_limits(uuid);

-- Returns as text instead of enum types
...
BEGIN
  -- Get user limits into a record
  SELECT * INTO found_row FROM public.get_user_kyc_limits(p_user_id);
  ...
  -- Check if verified
  IF found_row.kyc_status <> 'verified' THEN
    RETURN QUERY SELECT false, 'KYC verification required', 0::numeric, 0::numeric, 0::numeric;
    RETURN;
  END IF;
  
  -- Check trading permission
  IF p_transaction_type IN ('trade_buy', 'trade_sell') AND NOT found_row.can_trade THEN
    RETURN QUERY SELECT false, 'Trading not permitted at current KYC level', 0::numeric, 0::numeric, 0::numeric;
    RETURN;
  END IF;
  
  -- Check single transaction limit
  IF p_amount > found_row.max_single_trade THEN
    RETURN QUERY SELECT 
      false, 
      format('Exceeds single transaction limit of %s', found_row.max_single_trade),
      0::numeric, found_row.max_single_trade, 0::numeric;
    RETURN;
  END IF;
  ...
  CASE p_transaction_type
    WHEN 'deposit' THEN
      v_daily_limit_val := found_row.daily_deposit_limit;
      v_current_total := COALESCE(v_daily.deposits_total, 0);
    WHEN 'withdrawal' THEN
      v_daily_limit_val := found_row.daily_withdrawal_limit;
      v_current_total := COALESCE(v_daily.withdrawals_total, 0);
    WHEN 'trade_buy' THEN
      v_daily_limit_val := found_row.daily_trade_limit;
