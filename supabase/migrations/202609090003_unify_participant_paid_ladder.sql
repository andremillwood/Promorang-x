-- Unify participant paid membership onto Starter / Professional / Power User.
-- Earned Keys no longer grant paid cash-out or paid multipliers.
-- Membership keep is 95% platform / 5% PromoShare.

CREATE OR REPLACE FUNCTION public.normalize_participant_tier_key(p_tier text)
RETURNS text
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT CASE
    WHEN lower(regexp_replace(coalesce(p_tier, ''), '[^a-z0-9]+', '_', 'g'))
      IN ('elite', 'super', 'power', 'power_user', 'promorang_elite', 'participant_elite')
      THEN 'power_user'
    WHEN lower(regexp_replace(coalesce(p_tier, ''), '[^a-z0-9]+', '_', 'g'))
      IN (
        'plus', 'premium', 'pro', 'professional',
        'promorang_plus', 'participant_plus',
        'promorang_pro', 'participant_pro'
      )
      THEN 'professional'
    ELSE 'starter'
  END;
...
INSERT INTO public.participant_tier_configs (
  ...
  (
    ...
    0.0000,
    0.0000,
    '["withdraw_earnings", "no_gem_ceiling", "priority_access", "weekly_promokeys"]'::jsonb
  ),
  (
    'power_user',
    'Power User',
    ...
    0.0000,
    0.0000,
    '["withdraw_earnings", "fastest_master_key", "high_value_campaign_access"]'::jsonb
  )
...
UPDATE public.participant_tier_configs
SET
  display_name = 'Starter',
  is_paid = false,
  monthly_price_cents = 0,
  points_multiplier = 1.00,
  key_cost_multiplier = 1.00,
  promoshare_ticket_multiplier = 1.00,
  promoshare_weight_multiplier = 1.00,
  promoshare_cash_gem_eligible = false,
  monthly_cash_gem_cap = 300.00,
  monthly_key_unlock_threshold = NULL,
  promoshare_pool_percent = 0,
  liquidity_pool_percent = 0,
  local_impact_pool_percent = 0,
  benefits = '["points_and_keys", "non_cash_promoshare_entries", "basic_moment_access"]'::jsonb,
  updated_at = now()
WHERE tier_key IN ('free', 'starter');

UPDATE public.participant_tier_configs
SET
  display_name = CASE WHEN tier_key = 'professional' THEN 'Professional' ELSE display_name END,
  is_paid = true,
  ...
  promoshare_ticket_multiplier = 1.00,
  promoshare_weight_multiplier = 1.00,
  promoshare_cash_gem_eligible = true,
  monthly_cash_gem_cap = NULL,
  monthly_key_unlock_threshold = NULL,
  promoshare_pool_percent = 0.0500,
  liquidity_pool_percent = 0,
  local_impact_pool_percent = 0,
  benefits = '["withdraw_earnings", "no_gem_ceiling", "priority_access"]'::jsonb,
  updated_at = now()
WHERE tier_key IN ('plus', 'pro', 'professional');

UPDATE public.participant_tier_configs
SET
  display_name = CASE WHEN tier_key = 'power_user' THEN 'Power User' ELSE display_name END,
  is_paid = true,
  ...
  liquidity_pool_percent = 0,
  local_impact_pool_percent = 0,
  benefits = '["withdraw_earnings", "fastest_master_key", "high_value_campaign_access"]'::jsonb,
  updated_at = now()
WHERE tier_key IN ('elite', 'power_user');

-- Earned standing must not keep paid cash-out or paid multipliers.
UPDATE public.user_participant_tier_status
SET
  current_tier_key = 'starter',
  status_source = 'free',
  status = 'active',
  metadata = metadata || jsonb_build_object(
    'revoked_earned_paid_standing', true,
    'previous_tier_key', current_tier_key
  ),
  updated_at = now()
WHERE status_source = 'earned'
  AND current_tier_key IN ('plus', 'pro', 'elite', 'professional', 'power_user');

UPDATE public.user_participant_tier_status
SET
  current_tier_key = public.normalize_participant_tier_key(current_tier_key),
  updated_at = now()
WHERE current_tier_key IS NOT NULL
  AND current_tier_key <> public.normalize_participant_tier_key(current_tier_key);

UPDATE public.users
SET
  user_tier = public.normalize_participant_tier_key(user_tier),
  updated_at = now()
WHERE user_tier IS NOT NULL
  AND user_tier <> public.normalize_participant_tier_key(user_tier);

UPDATE public.participant_subscription_allocations
SET tier_key = public.normalize_participant_tier_key(tier_key)
WHERE tier_key IS NOT NULL
  AND tier_key <> public.normalize_participant_tier_key(tier_key);

DO $$
BEGIN
  IF to_regclass('public.promoshare_tickets') IS NOT NULL THEN
    UPDATE public.promoshare_tickets
    SET tier_key = public.normalize_participant_tier_key(tier_key)
    WHERE tier_key IS NOT NULL
      AND tier_key <> public.normalize_participant_tier_key(tier_key);
  END IF;
  ...
  IF to_regclass('public.promoshare_entries') IS NOT NULL THEN
    UPDATE public.promoshare_entries
    SET tier_key = public.normalize_participant_tier_key(tier_key)
    WHERE tier_key IS NOT NULL
      AND tier_key <> public.normalize_participant_tier_key(tier_key);
  END IF;
...
AS $$
  WITH raw_status AS (
    ...
    WHERE s.user_id = p_user_id
      AND s.status = 'active'
      AND s.status_source IN ('paid', 'admin')
      AND (
  ...
  effective_status AS (
    SELECT
      ...
      public.normalize_participant_tier_key(COALESCE((SELECT current_tier_key FROM raw_status), 'starter')) AS tier_key,
...
BEGIN
  ...
  VALUES (
    ...
    p_keys,
    p_source,
    coalesce(p_metadata, '{}'::jsonb) || jsonb_build_object('does_not_grant_paid_standing', true)
  )
...
DECLARE
  ...
  v_normalized_tier text := public.normalize_participant_tier_key(p_tier_key);
BEGIN
  IF p_amount IS NULL OR p_amount <= 0 THEN
    RAISE EXCEPTION 'p_amount must be positive';
  END IF;

  IF v_normalized_tier = 'starter' THEN
  ...
  WHERE tier_key = v_normalized_tier
    AND is_active = true
    AND is_paid = true;
