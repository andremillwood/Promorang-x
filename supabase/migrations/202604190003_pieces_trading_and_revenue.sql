...
-- Completes the multi-asset pieces market with trading, fees, and revenue sharing
...
CREATE TABLE IF NOT EXISTS public.piece_lockups (
  ...
  unlock_schedule jsonb NOT NULL, -- Array of {date, amount}
...
-- =====================================================
-- 8. CREATOR ECONOMICS INTEGRATION (Add piece trading revenue)
-- =====================================================

-- Add new source type to creator_revenue_source enum if not exists
DO $$
BEGIN
...
-- =====================================================
-- 9. PIECE PRICE ORACLES (Automated Price Feeds)
-- =====================================================
...
CREATE TABLE IF NOT EXISTS public.piece_price_oracles (
  ...
  data_sources jsonb NOT NULL, -- Array of {source, weight, metric}
...
-- =====================================================
-- 10. FUNCTIONS FOR REVENUE DISTRIBUTION
-- =====================================================

-- Function to calculate and distribute dividends
...
BEGIN
  ...
  -- Get total pieces in circulation
  v_total_pieces := (
    SELECT COALESCE(SUM(pieces_owned), 0)
    FROM public.content_piece_positions
    WHERE content_id = p_asset_id
  );
  ...
  SELECT 
    ...
    holder_id,
    pieces_owned,
    pieces_owned * v_dividend_per_piece
  FROM public.content_piece_positions
  WHERE content_id = p_asset_id;
