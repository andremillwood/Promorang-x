...
export default function TicketScannerScreen() {
    ...
    const { activateTicket } = useEventStore();
    ...
    const validateTicket = async (code: string) => {
        ...
        try {
            const data = await activateTicket(eventId, code);

            if (data.success) {
                Vibration.vibrate([0, 100, 50, 100]);
                setResult({ success: true, message: data.message || 'Ticket validated! Guest checked in.' });
            } else {
                Vibration.vibrate(500);
                setResult({ success: false, message: data.message || 'Invalid ticket code' });
            }
        } catch (error) {
