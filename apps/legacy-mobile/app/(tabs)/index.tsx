...
export default function DashboardScreen() {
  ...
  const { posts, unifiedFeed, isLoading: isFeedLoading, fetchPosts, fetchUnifiedFeed, likePost, sharePost } = useFeedStore();
  ...
  const { coupons, fetchCoupons, isLoading: isCouponsLoading } = useCouponStore();
  ...
  useEffect(() => {
    ...
    // Check if we should show onboarding nudge for State 0 users
    const checkOnboardingNudge = async () => {
      if (maturityState === UserMaturityState.FIRST_TIME && !isGuest) {
    ...
    checkOnboardingNudge();
  }, [maturityState, isGuest]);
  ...
  const getCurrentFeedData = () => {
    switch (activeTab) {
      case 'for-you':
        return unifiedFeed.length > 0 ? unifiedFeed : posts;
      case 'social':
      ...
      <Card style={styles.statsCard} variant="elevated">
        <View style={styles.statsRow}>
          ...
          <View style={styles.statItem}>
            ...
            <Text style={styles.usdValue}>≈ ${((user.gems_balance || 0) * 0.01).toFixed(2)}</Text>
          </View>
          <View style={styles.statItem}>
    ...
    <View style={styles.headerContainer}>
      ...
      {!isGuest && (
        <>
          <StreakWidget />
          <QuestsWidget />
          {/* Unlock Progress Bar for State 1 users */}
          {maturityState === UserMaturityState.EXPLORER && (
            <View style={{ marginBottom: 12 }}>
              <UnlockProgressBar completedQuests={(user as any)?.stats?.completed_quests || 0} />
            </View>
          )}
        </>
      )}
    ...
    switch (activeTab) {
      case 'drops':
        return {
          ...
          description: 'Check back later for new earning opportunities. In the meantime, see what\'s trending.',
          actionLabel: 'Scout Today',
          onAction: () => router.push('/today')
        };
      case 'products':
        return {
          title: 'No Products Yet',
          description: 'Check back later for new items in the shop. Want to earn more points first?',
      ...
      default:
        return {
          ...
          actionLabel: 'Check Today\'s Draw',
          onAction: () => router.push('/today')
        };
    }
  };
  ...
  const content = (
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      {/* Onboarding Nudge Modal for State 0 users */}
