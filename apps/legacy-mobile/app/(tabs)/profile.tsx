...
type TabType = 'overview' | 'content' | 'drops' | 'applications' | 'achievements' | 'store';
...
export default function ProfileScreen() {
  ...
  const fetchLeaderboardPosition = useCallback(async () => {
    if (!user?.id) return;
    try {
      const data = await api.get<any>(`/api/users/${String(user.id)}/leaderboard-position`);
  ...
  const fetchUserStore = useCallback(async () => {
    if (!user?.id) return;
    try {
      const data = await api.get<any>(`/api/marketplace/my-store`);
  ...
  const handleRefresh = async () => {
    ...
    await Promise.all([
      fetchPosts(),
      fetchLeaderboardPosition(),
      activeTab === 'drops' ? fetchMyDrops(String(user.id)) : Promise.resolve(),
      activeTab === 'applications' ? fetchMyApplications() : Promise.resolve(),
    ]);
    setRefreshing(false);
  };
  ...
  // Maturity Gating: State 0-3 see a simplified/locked profile view
  if (maturityState < UserMaturityState.OPERATOR_PRO) {
    const gatedContent = (
      <View style={[styles.container, { backgroundColor: theme.background }]}>
        <AppHeader showLogo title="Profile" />
        <ScrollView contentContainerStyle={styles.gatedContent}>
          <ProfileHeader
            user={user}
            isCurrentUser={true}
          ...
          <View style={styles.gatedCard}>
            <LinearGradient
              colors={[colors.primary + '20', '#8b5cf620']}
              style={styles.gatedBanner}
            >
              <Shield size={32} color={colors.primary} />
              <Text style={[styles.gatedTitle, { color: theme.text }]}>Identity Locked</Text>
              <Text style={[styles.gatedSubtitle, { color: theme.textSecondary }]}>
                Complete more Daily Layer activities to unlock your full creator profile and social commerce tools.
              </Text>
            </LinearGradient>

            <TouchableOpacity
              style={[styles.gatedAction, { backgroundColor: theme.surface, borderColor: theme.border }]}
              onPress={() => router.push('/(tabs)')}
            >
              <Text style={[styles.gatedActionText, { color: colors.primary }]}>Go to Daily Layer</Text>
              <ArrowRight size={18} color={colors.primary} />
            </TouchableOpacity>
          </View>

          <View style={styles.section}>
            <Text style={[styles.sectionTitle, { color: theme.text, marginLeft: 16 }]}>Limited Stats</Text>
            <View style={[styles.quickStatsGrid, { paddingHorizontal: 16 }]}>
              ...
              <Card style={[styles.quickStatCard, { backgroundColor: theme.card }]}>
                ...
                <Text style={[styles.quickStatLabel, { color: theme.textSecondary }]}>Quests</Text>
              </Card>
            </View>
          </View>
        </ScrollView>
    ...
    return isFocusedMode ? <TodayLayout>{gatedContent}</TodayLayout> : gatedContent;
  }
      ...
      renderItem={({ item }) => (
        <Card style={[styles.dropCard, { backgroundColor: theme.card }]}>
          ...
          <View style={styles.dropStats}>
            ...
            <Text style={[styles.dropStat, { color: theme.textSecondary }]}>
              {item.gem_reward_base || 0} Treasures reward
            </Text>
          </View>
        </Card>
      )}
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      <AppHeader showLogo showAvatar={false} />
      <FlatList
        data={[]}
        keyExtractor={() => 'header'}
        renderItem={() => null}
        ListHeaderComponent={
          <>
            ...
            <TabBar
              ...
              variant="underlined"
              containerStyle={StyleSheet.flatten([styles.tabBar, { backgroundColor: theme.surface, borderBottomColor: theme.border }])}
            />
          </>
        }
