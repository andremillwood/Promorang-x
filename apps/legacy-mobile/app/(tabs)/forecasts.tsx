...
export default function ForecastsScreen() {
  ...
  const { forecasts, myForecasts, isLoading: forecastsLoading, fetchForecasts, fetchMyForecasts } = useForecastStore();
  ...
  const handleForecastPress = (forecastId: string) => {
    router.push({ pathname: '/forecast/[id]', params: { id: forecastId } } as any);
  };
  ...
  const tabs = [
    { key: 'forecasts', label: 'Social Forecasts' },
    { key: 'myforecasts', label: 'My Forecasts' },
    { key: 'content', label: 'Content Shares' },
    { key: 'portfolio', label: 'Portfolio' },
    { key: 'growth', label: 'Growth Hub' },
  ];

  const isLoading = activeTab === 'forecasts' ? forecastsLoading :
    activeTab === 'myforecasts' ? forecastsLoading :
      activeTab === 'content' ? sharesLoading :
        activeTab === 'portfolio' ? sharesLoading : false;
  const data = activeTab === 'forecasts' ? forecasts :
    activeTab === 'myforecasts' ? myForecasts :
    ...
    if (activeTab === 'forecasts') {
      return (
        <FlatList
          ...
          renderItem={({ item }) => (
            <ForecastCard forecast={item} onPress={handleForecastPress} />
          )}
          ...
          ListEmptyComponent={
            <EmptyState
              ...
              description="Check back later for new social media forecasts."
      ...
      return (
        <FlatList
            ...
            <View style={styles.myBetItem}>
              ...
              <View style={styles.betStatus}>
                <View style={styles.betStatusRow}>
                  ...
                  <Text style={styles.betStatusValue}>$50.00 @ {item.odds}x odds</Text>
                </View>
                <View style={styles.betStatusRow}>
                  <Text style={styles.betStatusLabel}>Potential Win:</Text>
                  <Text style={[styles.betStatusValue, styles.winAmount]}>${(50 * item.odds).toFixed(2)}</Text>
                </View>
                <View style={styles.betStatusRow}>
          ...
          ListEmptyComponent={
            <EmptyState
              title="No Forecasts Made"
              description="Make your first prediction to start tracking your positions."
              icon={<TrendingUp size={48} color={colors.darkGray} />}
              style={styles.emptyState}
            />
          }
      ...
      const totalInvested = myOwnerships.reduce((sum: number, ownership: any) => sum + ownership.totalInvested, 0);
      const totalDividends = myOwnerships.reduce((sum: number, ownership: any) => sum + ownership.dividendsEarned, 0);
      const currentValue = myOwnerships.reduce((sum: number, ownership: any) => {
        const share = contentShares.find((s: any) => s.id === ownership.contentShareId);
        ...
        <View style={styles.portfolioContainer}>
          ...
          <FlatList
            ...
            renderItem={({ item }: { item: any }) => {
              const share = contentShares.find((s: any) => s.id === item.contentShareId);
