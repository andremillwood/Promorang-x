...
function RootLayoutNav() {
  ...
  const [maturityState, setMaturityState] = useState<number | null>(null);
  ...
  useEffect(() => {
    ...
    // Check maturity state for authenticated users
    const checkMaturityState = async () => {
      if (!isAuthenticated && !isGuest) return;
      try {
        const response = await apiClient.get('/users/me');
        if (response?.maturity_state !== undefined) {
          setMaturityState(response.maturity_state);
        } else {
          setMaturityState(1); // Default to state 1 if unavailable
        }
      } catch (e) {
        setMaturityState(1); // Default on error
      }
    };
    checkMaturityState();
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      ...
      if (isAuthenticated || isGuest) {
        if (inAuthGroup) {
          // Check if user needs to complete first quest (State 0)
          if (maturityState === 0) {
            router.replace('/onboarding/first-quest' as any);
  ...
  }, [isAuthenticated, isGuest, hasSeenWelcome, segments, rootNavigationState?.key, hasCompletedQuestionnaire, maturityState]);
  ...
  return (
    <Stack>
      ...
      <Stack.Screen name="onboarding/first-quest" options={{ headerShown: false, gestureEnabled: false }} />
      ...
      <Stack.Screen name="events-entry/index" options={{ headerShown: false }} />
      <Stack.Screen name="post-entry/index" options={{ headerShown: false }} />
      <Stack.Screen name="start/index" options={{ headerShown: false }} />
      <Stack.Screen name="today/index" options={{ headerShown: false }} />
      <Stack.Screen name="today/opportunity" options={{ headerShown: false }} />
