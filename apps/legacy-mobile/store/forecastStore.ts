...
export const useForecastStore = create<ForecastState>((set, get) => ({
    ...
    fetchForecasts: async () => {
        ...
        try {
            const data = await api.get<any>('/api/social-forecasts');
            const dataArray = Array.isArray(data) ? data : (data.data || []);

            const mappedForecasts: Forecast[] = dataArray.map((item: any) => ({
                id: item.id?.toString(),
    ...
    fetchMyForecasts: async () => {
        ...
        try {
            const data = await api.get<any>('/api/portfolio/predictions');
    ...
    makePrediction: async (forecastId: string, amount: number, prediction: boolean) => {
        ...
        try {
            ...
            const data = await api.post<any>(`/api/social-forecasts/${forecastId}/predict`, payload);
            if (!data.success && data.status !== 'success') throw new Error(data.error || 'Prediction failed');
