...
export const useTaskStore = create<TaskState>((set, get) => ({
  ...
  fetchTasks: async () => {
    ...
    try {
      const data = await api.get<any[]>('/api/drops');
  ...
  fetchMyTasks: async () => {
    set({ isLoading: true });
    try {
      const data = await api.get<{ success: boolean; applications?: any[] }>('/api/drops/my-applications');
  ...
  applyForTask: async (taskId: string, applicationMessage: string = 'Applied from mobile app') => {
    ...
    try {
      const data = await api.post<{ success: boolean; error?: string }>(`/api/drops/${taskId}/apply`, {
  ...
  completeTask: async (taskId: string, submissionUrl?: string, submissionNotes?: string) => {
    ...
    try {
      const data = await api.post<{ success: boolean; error?: string }>(`/api/drops/${taskId}/submit`, {
