...
export interface Campaign {
    ...
    campaign_maturity?: 'seed' | 'activated' | 'funded' | 'dominant';
...
export const useAdvertiserStore = create<AdvertiserState>((set, get) => ({
    ...
    fetchDashboard: async () => {
        set({ isLoading: true });
        try {
            const data = await api.get<any>('/api/advertisers/dashboard');
    ...
    fetchSuggestedContent: async () => {
        ...
        try {
            const data = await api.get<any>('/api/advertisers/suggested-content');
    ...
    fetchCampaigns: async () => {
        ...
        try {
            const data = await api.get<any>('/api/advertisers/campaigns');
    ...
    fetchCoupons: async () => {
        ...
        try {
            const data = await api.get<any>('/api/advertisers/coupons');
            if (data.status === 'success' && data.data) {
                set({
    ...
    fetchPlans: async () => {
        try {
            const data = await api.get<any>('/api/advertisers/subscription/plans');
    ...
    createCoupon: async (coupon) => {
        try {
            const data = await api.post<any>('/api/advertisers/coupons', coupon);
    ...
    assignCoupon: async (couponId, assignment) => {
        try {
            const data = await api.post<any>(`/api/advertisers/coupons/${couponId}/assign`, assignment);
    ...
    redeemCoupon: async (couponId, userName) => {
        try {
            const data = await api.post<any>(`/api/advertisers/coupons/${couponId}/redeem`, { user_name: userName });
            if (data.status === 'success') {
                get().fetchCoupons();
                return true;
            }
    ...
    getCoupon: async (couponId) => {
        try {
            const data = await api.get<any>(`/api/advertisers/coupons/${couponId}`);
            if (data.status === 'success' && data.data?.coupon) {
    ...
    updateCoupon: async (couponId, updates) => {
        try {
            const data = await api.put<any>(`/api/advertisers/coupons/${couponId}`, updates);
            if (data.status === 'success' && data.data?.coupon) {
    ...
    deleteCoupon: async (couponId) => {
        try {
            const data = await api.delete<any>(`/api/advertisers/coupons/${couponId}`);
            if (data.status === 'success') {
                set((state) => ({
    ...
    replenishCoupon: async (couponId, quantity) => {
        try {
            const data = await api.post<any>(`/api/advertisers/coupons/${couponId}/replenish`, { quantity });
            if (data.status === 'success' && data.data?.coupon) {
                const updatedCoupon = data.data.coupon;
                set((state) => ({
                    coupons: state.coupons.map(c => c.id === couponId ? updatedCoupon : c),
    ...
    toggleCouponStatus: async (couponId) => {
        try {
            ...
            const data = await api.put<any>(`/api/advertisers/coupons/${couponId}`, { status: newStatus });
            if (data.status === 'success') {
                set((state) => ({
    ...
    removeAssignment: async (couponId, assignmentId) => {
        try {
            const data = await api.delete<any>(`/api/advertisers/coupons/${couponId}/assignments/${assignmentId}`);
            if (data.status === 'success') {
                get().fetchCoupons();
    ...
    createCampaign: async (campaign) => {
        try {
            const data = await api.post<any>('/api/advertisers/campaigns', campaign);
    ...
    getCampaign: async (campaignId) => {
        try {
            const data = await api.get<any>(`/api/advertisers/campaigns/${campaignId}`);
            if (data.status === 'success' && data.data?.campaign) {
    ...
    updateCampaign: async (campaignId, updates) => {
        try {
            const data = await api.put<any>(`/api/advertisers/campaigns/${campaignId}`, updates);
            if (data.status === 'success' && data.data?.campaign) {
    ...
    deleteCampaign: async (campaignId) => {
        try {
            const data = await api.delete<any>(`/api/advertisers/campaigns/${campaignId}`);
            if (data.status === 'success') {
                set((state) => ({
    ...
    updateCampaignStatus: async (campaignId, status) => {
        try {
            const data = await api.put<any>(`/api/advertisers/campaigns/${campaignId}/status`, { status });
            if (data.status === 'success') {
                set((state) => ({
    ...
    addCampaignFunds: async (campaignId, amount) => {
        try {
            const data = await api.post<any>(`/api/advertisers/campaigns/${campaignId}/funds`, { amount });
            if (data.status === 'success' && data.data) {
    ...
    getCampaignContent: async (campaignId) => {
        try {
            const data = await api.get<any>(`/api/advertisers/campaigns/${campaignId}/content`);
            if (data.status === 'success' && Array.isArray(data.data)) {
                set({ campaignContent: data.data });
                return data.data;
            }
            return [];
        } catch (error) {
    ...
    addCampaignContent: async (campaignId, content) => {
        try {
            const data = await api.post<any>(`/api/advertisers/campaigns/${campaignId}/content`, content);
            if (data.status === 'success' && data.data) {
