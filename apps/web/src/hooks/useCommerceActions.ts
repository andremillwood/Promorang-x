...
export function useCommerceActions(){
  ...
  const claim=async(couponId:string)=>{setBusy('claim');try{const r=await fetch(`${API_BASE_URL}/coupons/${couponId}/redeem`,{method:'POST',headers:await auth({kind:'offer_claim',targetId:couponId})});const d=await r.json();if(!r.ok)throw new Error(d.message||'Claim failed');clearResumableIntent({kind:'offer_claim',targetId:couponId});toast({title:'Offer saved to your wallet',description:d.data?.redemption?.redemption_code?`Code ${d.data.redemption.redemption_code}`:'Ready to redeem'});return d}catch(e:any){if(e?.message===AUTH_REDIRECT)return undefined;throw e}finally{setBusy(null)}};
