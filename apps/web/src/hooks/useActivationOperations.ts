...
export type Collaborator = { id: string; role: string; display_name: string | null; invited_user_id: string | null; invited_organization_id: string | null; responsibility: string | null; status: string; response_message: string | null };
...
export function useActivationOperations(proposalId: string) {
  ...
  const people = useQuery({ queryKey: ["activation-people"], queryFn: async () => { const { data, error } = await db.from("profiles").select("user_id,full_name,location,avatar_url").not("full_name", "is", null).order("full_name").limit(50); if (error) throw error; return (data || []) as PersonOption[]; } });
  const operations = useQuery({ queryKey: ["activation-operations", proposalId], queryFn: async () => {
    const user = (await db.auth.getUser()).data.user;
    const [collaborators, content, contributions, history, outcomes, access, passes, funding, payouts, cases, reserve, reservations, wallet] = await Promise.all([
    ...
    const error = collaborators.error || content.error || contributions.error || history.error || outcomes.error || access.error || passes.error || funding.error || payouts.error || cases.error || reserve.error || reservations.error || wallet.error;
    if (error) throw error;
    return { collaborators: (collaborators.data || []) as Collaborator[], content: (content.data || []) as ContentAssignment[], contributions: (contributions.data || []) as Contribution[], history: history.data || [], outcomes: (outcomes.data || []) as OutcomeSnapshot[], access: (access.data || []) as AccessTier[], passes: (passes.data || []) as AccessPass[], funding: (funding.data || []) as FundingEvent[], payouts: (payouts.data || []) as PayoutAllocation[], cases: (cases.data || []) as ResolutionCase[], reserve: reserve.data as GemReserve | null, reservations: (reservations.data || []) as GemReservation[], wallet: wallet.data as EconomyWallet | null };
  ...
  return { scenes, people, operations, linkScene, linkMoment, move, invite, addContent, addContribution, respond, recordOutcome, addAccess, claimAccess, purchaseAccess, secureGems, addPayout, openCase, releasePayout, refundReservation };
}
