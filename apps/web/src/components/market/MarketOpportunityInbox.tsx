...
function responseFor(role: DemandRole) {
  switch (role) {
    case "merchant": return { href: "/give", label: "Put up an offer", note: "Make the terms and availability clear." };
    case "brand": return { href: "/dashboard?view=studio&tab=campaigns", label: "Build a brand response", note: "Choose what you want people to do next." };
    case "host": return { href: "/create/moment", label: "Create a Moment", note: "Give people a clear time, place and reason to show up." };
    default: return { href: "/create?intent=answer", label: "Create a response", note: "Give people a next move they can actually take." };
  }
}
...
export function MarketOpportunityInbox({ role }: { role: DemandRole }) {
  ...
  if (isLoading) return <p className="py-8 text-sm text-white/40">Loading market opportunities…</p>;
    ...
    <div className="space-y-10">
      <section className="grid gap-4 lg:grid-cols-[1fr_.65fr] lg:items-end">
        <div>
          ...
          <h2 className="mt-3 font-serif text-4xl font-bold tracking-[-.04em] sm:text-5xl">What people want that you may be able to answer.</h2>
          <p className="mt-4 max-w-2xl text-sm leading-7 text-white/50">Use these asks and votes as a starting point. Decide whether they fit your audience, capacity and goals before you respond.</p>
        </div>
        <div className="rounded-[1.6rem] border border-white/10 bg-white/[0.03] p-5">
          <p className="text-[10px] font-black uppercase tracking-[0.16em] text-white/35">What the market is saying</p>
      ...
      <section>
        <div className="mb-5 border-b border-white/10 pb-4">
          ...
          <h3 className="mt-2 font-serif text-3xl font-bold">Things people asked for that do not yet have a matching live question.</h3>
        </div>
        {inbox.misses.length ? <div className="grid gap-3 sm:grid-cols-2">{inbox.misses.map((ask) => (
          <article key={ask.query} className="rounded-[1.4rem] border border-white/10 bg-white/[0.025] p-5">
            ...
            <p className="mt-2 text-xs leading-5 text-white/40">{ask.count} ask{ask.count === 1 ? "" : "s"}. There isn’t a matching live question yet.</p>
          </article>
      ...
      <section>
        <div className="mb-5 border-b border-white/10 pb-4">
          <p className="font-mono text-[10px] font-black uppercase tracking-[0.18em] text-primary">People are asking for</p>
          <h3 className="mt-2 font-serif text-3xl font-bold">Questions people are actively voting on.</h3>
        </div>
        {inbox.questions.length ? <div className="space-y-4">{inbox.questions.map((question) => {
          ...
          return (
            <article key={question.poll.id} className="grid gap-5 rounded-[1.6rem] border border-white/10 bg-white/[0.025] p-5 lg:grid-cols-[1fr_auto] lg:items-center">
              <div>
                <div className="flex flex-wrap items-center gap-2 text-[10px] font-black uppercase tracking-[0.14em] text-white/35"><Radio className="h-3.5 w-3.5 text-primary" />Demand · {question.poll.totalVotes || 0} votes {thresholdMet ? "· target reached" : question.poll.thresholdForMoment ? `· ${question.votesRemaining} more to target` : ""}</div>
                <h4 className="mt-3 font-serif text-2xl font-bold">{question.poll.question}</h4>
                {question.leading ? <p className="mt-2 text-sm text-white/50">Most people are saying: <strong className="text-white/75">{question.leading.text}</strong> · {question.leading.votes} votes</p> : null}
              ...
              <div className="flex flex-wrap gap-2 lg:flex-col lg:items-stretch">
                <Link to={href} className="inline-flex min-h-10 items-center justify-center gap-2 rounded-full border border-white/15 px-4 text-xs font-black">Open signal <ArrowRight className="h-3.5 w-3.5" /></Link>
        ...
        })}</div> : <div className="rounded-[1.4rem] border border-dashed border-white/10 p-6"><Users className="h-5 w-5 text-primary" /><p className="mt-3 font-serif text-2xl font-bold">No live questions yet.</p><p className="mt-2 text-sm text-white/40">Nothing needs a response here right now. Keep watching as the market moves.</p></div>}
      </section>
    </div>
  );
}
