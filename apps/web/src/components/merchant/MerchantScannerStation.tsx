...
export function MerchantScannerStation({ venueName = "Merchant station" }: { venueName?: string }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      const result = await redeem.mutateAsync({ code: normalized });
      setLastRedemption(result);
      setCode("");
      toast({ title: "Perk redeemed", description: result.offers?.title || "The redemption was recorded." });
    } catch (error) {
      toast({ title: "Could not redeem this code", description: error instanceof Error ? error.message : "Check the code and try again.", variant: "destructive" });
    }
  };
    ...
    <section className="rounded-3xl border border-zinc-800 bg-zinc-900/90 p-6 text-white shadow-2xl sm:p-8">
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-400">Enter the code shown on the member’s PromoCard. A success message means the redemption was recorded.</p>
        </div>
      </div>
      <form onSubmit={submit} className="mt-6 flex flex-col gap-3 sm:flex-row">
        <Input value={code} onChange={(event) => setCode(event.target.value)} placeholder="PR-XXXXXXXX" autoCapitalize="characters" autoComplete="off" className="h-12 border-zinc-700 bg-zinc-950 font-mono uppercase tracking-widest" aria-label="PromoCard redemption code" />
        <Button type="submit" disabled={!code.trim() || redeem.isPending} className="h-12 bg-emerald-500 font-black text-zinc-950 hover:bg-emerald-400"><KeyRound className="mr-2 h-4 w-4" />{redeem.isPending ? "Checking…" : "Redeem"}</Button>
      </form>
      {lastRedemption ? (
