...
export function MobilePromoHome({ offers, moments }: { offers: MobileOffer[]; moments: MobileMoment[] }) {
  ...
  const displayOffers = offers.length ? offers.slice(0, 4) : [
    { id: "food", title: t("home.mobileFallbackFood"), merchant: t("home.mobileFallbackFoodMerchant"), image: "https://images.unsplash.com/photo-1551504734-5ee1c4a1479b?auto=format&fit=crop&q=80&w=900", href: "/shop" },
    { id: "night", title: t("home.mobileFallbackNight"), merchant: t("home.mobileFallbackNightMerchant"), image: "https://images.unsplash.com/photo-1514525253161-7a46d19cd819?auto=format&fit=crop&q=80&w=900", href: "/discover" },
  ];
    ...
    <div className="bg-[#f3efe6] text-[#14120f] md:hidden">
      <section className="px-5 py-12">
        <div className="flex items-end justify-between gap-4">
          <div>
            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#a34a1a]">{t("home.mobileUseNearby")}</p>
            <h2 className="mt-2 max-w-[15rem] font-serif text-[2rem] font-black leading-[0.96] tracking-[-0.04em]">
              {t("home.mobileNextOuting")}
            </h2>
          </div>
          <Link to="/shop" className="mb-1 shrink-0 text-xs font-black text-[#a34a1a]">{t("home.mobileSeeAll")}</Link>
        </div>
          ...
          {displayOffers.map((offer) => (
            <Link key={offer.id} to={offer.href} className="group w-[78vw] max-w-[19rem] shrink-0 snap-start overflow-hidden rounded-[1.35rem] bg-[#171512] shadow-[0_18px_45px_rgba(46,31,17,0.16)]">
              <div className="relative aspect-[16/10] overflow-hidden">
                ...
                <span className="absolute left-3 top-3 rounded-full bg-[#f3efe6]/95 px-2.5 py-1 text-[9px] font-black uppercase tracking-[0.12em] text-[#3d2a1e]">{t("home.mobilePromoPlace")}</span>
              </div>
              <div className="p-4 text-white">
                ...
                <div className="mt-4 flex items-center justify-between border-t border-white/10 pt-3">
                  <span className="text-xs text-white/60">{t("home.mobileViewBefore")}</span>
        ...
        <p className="mt-3 text-xs leading-5 text-[#625a50]">{t("home.mobileOffersNote")}</p>
      </section>

      <section className="bg-[#171512] px-5 py-14 text-white">
        <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#f6ad72]">{t("home.mobileLoopEyebrow")}</p>
        <h2 className="mt-2 max-w-xs font-serif text-[2rem] font-black leading-[0.98] tracking-[-0.04em]">{t("home.mobileLoopTitle")}</h2>
        <div className="mt-8 space-y-7">
          {[
            ["01", ScanLine, t("home.mobileS1Title"), t("home.mobileS1Copy")],
            ["02", MapPin, t("home.mobileS2Title"), t("home.mobileS2Copy")],
            ["03", Zap, t("home.mobileS3Title"), t("home.mobileS3Copy")],
            ...
            return (
              <div key={number as string} className="grid grid-cols-[2.75rem_1fr] gap-3">
                ...
                <div className="border-b border-white/10 pb-7">
                  <p className="text-[9px] font-black tracking-[0.18em] text-white/35">{t("home.mobileStep", { number: number as string })}</p>
      ...
      {moments.length > 0 && (
        <section className="px-5 py-12">
          <div className="flex items-end justify-between gap-4">
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#a34a1a]">{t("home.mobileNearbyEyebrow")}</p>
              <h2 className="mt-2 font-serif text-[2rem] font-black leading-none tracking-[-0.04em]">{t("home.mobileNearbyTitle")}</h2>
            </div>
            <Link to="/discover" className="mb-1 shrink-0 text-xs font-black text-[#a34a1a]">{t("home.mobileDiscover")}</Link>
          </div>
            ...
            {moments.slice(0, 3).map((moment) => (
              <Link key={moment.id} to={moment.href} className="grid min-h-[6rem] grid-cols-[5.5rem_1fr] overflow-hidden rounded-[1.2rem] bg-white shadow-[0_10px_30px_rgba(46,31,17,0.08)] active:scale-[0.99]">
                ...
                <div className="min-w-0 p-3.5">
                  <p className="text-[9px] font-black uppercase tracking-[0.12em] text-[#a34a1a]">{moment.date || t("home.comingUp")}</p>
      ...
      <section className="px-5 pb-[calc(7rem+env(safe-area-inset-bottom,0px))] pt-4">
        <div className="rounded-[1.5rem] bg-[#e8ddc9] p-5">
          ...
          <h2 className="mt-5 font-serif text-2xl font-black leading-tight">{t("home.mobileReadyTitle")}</h2>
          <ul className="mt-4 space-y-2 text-sm text-[#51483e]">
            {[t("home.mobileBenefit1"), t("home.mobileBenefit2"), t("home.mobileBenefit3")].map((item) => <li key={item} className="flex items-center gap-2"><Check className="h-4 w-4 text-emerald-700" />{item}</li>)}
          </ul>
          <Link to={primaryHref} className="mt-6 flex min-h-12 items-center justify-center gap-2 rounded-xl bg-[#171512] px-5 text-sm font-black text-white active:scale-[0.98]">
            {user ? t("home.promoOpenCard") : t("home.promoGetCard")}<ArrowRight className="h-4 w-4" />
          </Link>
        </div>
      </section>
        ...
        <div className="fixed inset-x-0 bottom-0 z-30 border-t border-black/10 bg-[#f3efe6]/95 px-4 pb-[max(.65rem,env(safe-area-inset-bottom))] pt-2.5 backdrop-blur-xl">
          <div className="mx-auto grid max-w-md grid-cols-[1fr_auto] gap-2">
            <Link to={primaryHref} className="flex min-h-12 items-center justify-center gap-2 rounded-xl bg-[#ed5a10] px-4 text-sm font-black text-white shadow-[0_12px_28px_rgba(237,90,16,0.28)] active:scale-[0.98]">
              <WalletCards className="h-4 w-4" />{t("home.promoGetCard")}
            </Link>
            <Link to="/shop" aria-label={t("home.mobileFindPlaces")} className="grid min-h-12 min-w-12 place-items-center rounded-xl border border-black/10 bg-white text-[#171512] active:bg-black/5"><MapPin className="h-5 w-5" /></Link>
          </div>
        </div>
      )}
    </div>
