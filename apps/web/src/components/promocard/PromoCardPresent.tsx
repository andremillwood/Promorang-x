      ...
      {!compact && surface.perks.length ? (
        <section className="space-y-3">
          ...
          {surface.perks.map((perk) => (
            <article key={perk.id} className="space-y-2">
              <TicketPass
                kicker="Perk"
                title={perk.title}
                detail={perk.detail || "Show this with your PromoCard."}
                stub={perk.redemptionCode ? perk.redemptionCode.slice(-4) : "Keep"}
                stubLabel="Hold"
              />
              {perk.redemptionCode ? (
                <p className="px-1 font-mono text-sm tracking-[0.16em] text-amber-100">
                  Show this code: {perk.redemptionCode}
                </p>
              ) : null}
              {perk.expiresAt ? (
                <p className="px-1 text-xs text-white/40">Expires {new Date(perk.expiresAt).toLocaleDateString()}</p>
              ) : null}
            </article>
          ))}
        </section>
