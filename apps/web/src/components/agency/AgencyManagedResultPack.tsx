      ...
      <div className="rounded-[2rem] border border-border/70 bg-card p-5 sm:p-7">
        ...
        <div className="mt-3 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">This view only summarizes records attached to connected client workspaces. A redemption is evidence of a recorded customer action; it is not automatically a sale, ROI claim, or incremental lift.</p>
          </div>
          <Badge variant="outline" className="w-fit">{brandClients.length} brand client{brandClients.length === 1 ? "" : "s"}</Badge>
        </div>
        <div className="mt-6 grid gap-3 sm:grid-cols-3">
          {[
            ["Active client work", activeCampaigns, Layers3],
            ["Campaigns with recorded action", provenCampaigns, CheckCircle2],
            ["Recorded redemptions", totalRedemptions, TrendingUp],
      ...
      {agencyClients.length === 0 ? (
        ...
        <div className="space-y-3">
          {agencyClients.map((client) => {
            const clientCampaigns = campaigns.filter((campaign) => campaign.organization_id === client.id);
            const clientRedemptions = clientCampaigns.reduce((sum, campaign) => sum + Number(campaign.redemptions || 0), 0);
            const ClientIcon = client.type === "merchant" ? Store : Building2;
            return (
              <article key={client.id} className="rounded-3xl border border-border/70 bg-card p-5 sm:p-6">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                  <div className="flex gap-3">
                    ...
                    <div><p className="font-serif text-xl font-semibold">{client.name}</p><p className="mt-1 text-xs text-muted-foreground">{client.type === "merchant" ? "Venue client" : "Brand client"} · {clientCampaigns.length} recorded campaign{clientCampaigns.length === 1 ? "" : "s"}</p></div>
                  </div>
                  <Button size="sm" variant="outline" onClick={() => { setActiveOrgId(client.id); setActiveRole(client.type === "merchant" ? "merchant" : "brand"); }}>Open client workspace <ArrowRight className="ml-2 h-4 w-4" /></Button>
                </div>
