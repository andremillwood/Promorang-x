    ...
    <aside className="hidden lg:flex flex-col gap-5 w-80 shrink-0 sticky top-20 h-fit">
      <div className="rounded-3xl border border-white/10 bg-gradient-to-br from-white/[0.05] via-[#121316] to-black p-5 shadow-xl backdrop-blur-md">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div>
            <h3 className="text-sm font-bold text-white">{t("discover.railMapTitle", { city })}</h3>
            <p className="text-[11px] text-white/50">{t("discover.railMapSub")}</p>
          </div>
        </div>
        <p className="text-xs text-white/70 mb-4 leading-relaxed">
          {t("discover.railMapHint", { city })}
        </p>
        <Button
          onClick={onToggleMap}
        ...
        >
          ...
          <span>{isMapMode ? t("discover.showGrid") : t("discover.exploreMap")}</span>
        </Button>
      </div>
            ...
            >
              <div className="space-y-0.5 min-w-0 pr-2">
                ...
                <p className="text-[11px] text-white/50 flex items-center gap-1">
                  ...
                  <span className="truncate">{m.venue_name || m.location || city}</span>
                </p>
              </div>
      ...
      <div className="rounded-3xl border border-white/10 bg-white/[0.02] p-4 text-xs text-white/60 space-y-1">
        <div className="flex items-center gap-1.5 text-white font-bold">
          ...
          <span>{t("discover.promoTip")}</span>
        </div>
        <p className="text-[11px] leading-relaxed text-white/50">
          {t("discover.promoTipCopy")}
        </p>
      </div>
    </aside>
  );
