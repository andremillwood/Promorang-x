...
function questionAction(role: DemandRole, to: (path: string) => string): { href: string; label: TranslationKey } {
...
function QuestionCard({
  ...
  const action = questionAction(role, to);
  const closenessKey =
...
export function DiscoveryDemandInbox({
  ...
  if (variant === "peek") {
    return (
      <Link
        to={to("/demand")}
        className="block rounded-[1.6rem] border border-white/10 bg-white/[0.04] px-5 py-5"
      >
  ...
  return (
    <div className="space-y-6">
      <header className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(220px,280px)] lg:items-center">
        <div className={cn("rounded-[1.6rem] border bg-gradient-to-r via-black to-black p-5 sm:p-6", tone.ring)}>
        ...
        <PaperReceipt
          heading={t("demand.receiptHeading")}
          lines={[
