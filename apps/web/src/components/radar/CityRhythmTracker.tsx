...
import { Calendar, Flame, Sparkles, Key, CheckCircle2, TrendingUp, ArrowRight, ShieldCheck } from 'lucide-react';
...
const RHYTHM_STEPS = [
  {
    ...
    day: 'MON - TUE',
    title: 'Debate & Signal',
    description: 'Vote on weekly discoveries & spicy city debates',
    icon: Flame,
  ...
  },
  {
    ...
    description: 'Promorang presents voter proof to #1 spot',
    icon: TrendingUp,
  ...
  },
  {
    ...
    day: 'WEDNESDAY',
    title: 'Keys Drop',
    description: '15 limited VIP passes unlock for top voters',
    icon: Key,
  ...
  },
  {
    ...
    day: 'FRI - SAT',
    title: 'Live Moments',
    description: 'Go out, scan QR codes, claim perks & earn points',
    icon: Sparkles,
  ...
  },
  {
    ...
    day: 'SUNDAY',
    title: 'Recap & Ledger',
    description: 'Weekly leaderboard proof & merchant renewals',
    icon: ShieldCheck,
    ...
    <div className="rounded-3xl bg-gradient-to-b from-gray-900/90 via-gray-950 to-gray-950 border border-gray-800 p-5 md:p-6 shadow-2xl mb-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 pb-4 mb-5 border-b border-gray-800/80">
        ...
        <div className="flex items-center space-x-2 bg-gray-900 border border-gray-800 px-3.5 py-1.5 rounded-2xl w-fit">
          ...
          <span className="text-xs font-black text-purple-300">
            {currentStage === 'DISCOVER' ? '🔥 Debate & Demand Active' : '✨ Weekend Activations Live'}
          </span>
        </div>
      </div>
      ...
      <div className="mt-5 p-3.5 bg-gradient-to-r from-purple-950/40 via-gray-900/60 to-orange-950/40 rounded-2xl border border-gray-800 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
        <div className="flex items-center space-x-2.5 text-gray-300">
          ...
          <span>
            <strong className="text-white">Active Debate:</strong> {activeDebateTitle} ({keysCount} Keys Unlocking Wednesday)
          </span>
        </div>
        <button
          onClick={() => onNavigateTab && onNavigateTab('DISCOVER')}
          className="text-orange-400 hover:text-orange-300 font-black text-xs flex items-center space-x-1 shrink-0"
        >
          <span>Vote to Influence Supply</span>
