...
export function HostSponsorshipRequests() {
    ...
    return (
      <div className="space-y-6">
        <div className="bg-card rounded-xl p-8 border border-border text-center">
          ...
          <h3 className="font-semibold text-foreground mb-2">{t("sponReq.emptyTitle")}</h3>
        ...
        <div>
           <div className="flex items-center gap-2 mb-4">
               <h3 className="font-semibold text-foreground">{t("sponReq.matchTitle")}</h3>
               <Badge variant="outline" className="text-[10px] text-muted-foreground uppercase tracking-widest bg-muted/50">{t("sponReq.comingSoon")}</Badge>
           </div>
           ...
           <div className="pointer-events-none rounded-xl border border-border/50 bg-gradient-warm p-4 opacity-80 sepia-[0.2] filter sm:p-6">
                <div className="mb-4 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                  ...
                  <div className="text-left sm:text-right">
                    ...
                    <Badge className="bg-blue-500/10 text-blue-500 border-blue-500/20">{t("sponReq.openBounty")}</Badge>
                  </div>
                </div>

                <div className="mb-4">
                  <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-1">{t("sponReq.requirements")}</p>
  ...
  return (
    <div className="space-y-10">
      ...
      {requests.filter(r => r.status === "funded").length > 0 && (
        <section className="animate-in fade-in slide-in-from-bottom-4 duration-500">
          <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="font-serif text-xl font-bold text-foreground">{t("sponReq.activeTitle")}</h3>
          </div>
               ...
               <div key={request.id} className="bg-emerald-500/5 border border-emerald-500/20 rounded-2xl p-6 relative overflow-hidden group">
                  ...
                  <div className="relative z-10 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                     <div className="min-w-0 space-y-4">
                        <div>
                           <Badge className="bg-emerald-500 text-white mb-2 shadow-glow-emerald">{t("sponReq.fundedLocked")}</Badge>
                           <h4 className="text-2xl font-black italic font-serif">
                              {request.moment?.title || t("sponReq.communityMoment")}
                           </h4>
                           <p className="text-sm text-foreground/60 font-medium">{t("sponReq.sponsoredBy")}</p>
                        </div>
                        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:gap-6">
                           <div>
                              <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">{t("sponReq.momentBudget")}</p>
                           ...
                           <div>
                              <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">{t("sponReq.hostPayouts")}</p>
                              <p className="text-[10px] font-bold text-foreground uppercase tracking-wider bg-emerald-500/10 px-2 py-0.5 rounded">{t("sponReq.verifiedPayouts")}</p>
                           </div>
                        </div>
                     </div>
      ...
      {pendingRequests.length > 0 && (
        <div>
          <h3 className="font-semibold text-foreground mb-4">
            {t("sponReq.pendingTitle", { count: pendingRequests.length })}
          </h3>
          <div className="space-y-4">
            {pendingRequests.map((request) => (
              ...
              >
                <div className="mb-3 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  <div className="min-w-0">
                    <p className="font-medium text-foreground">
                      {t("sponReq.brand", { id: request.brand_id.slice(0, 8) })}
                    </p>
                    <p className="text-sm text-muted-foreground">
                      {t("sponReq.forMoment", { id: request.moment_id.slice(0, 8) })}
                    </p>
                  </div>
                  <div className="text-left sm:text-right">
                ...
                {request.requirements && (
                  <div className="mb-3">
                    <p className="text-xs text-muted-foreground mb-1">{t("sponReq.requirements")}</p>
                ...
                <div className="mb-4 flex flex-col gap-2 text-xs text-muted-foreground sm:flex-row sm:items-center sm:justify-between">
                  <span>{t("sponReq.received", { when: formatDate(request.created_at, { month: "short", day: "numeric", year: "numeric" }) })}</span>
                  {request.expires_at && (
                    <span className="flex items-center gap-1">
                      <Clock className="w-3 h-3" />
                      {t("sponReq.expires", { when: formatDate(request.expires_at, { month: "short", day: "numeric" }) })}
                    </span>
                  )}
                </div>
                ...
                <div className="flex flex-col gap-2 sm:flex-row">
                  ...
                  <Dialog>
                    ...
                    <DialogContent>
                      <DialogHeader>
                        <DialogTitle>{t("sponReq.dialogTitle")}</DialogTitle>
                        <DialogDescription>
                          {t("sponReq.dialogDesc")}
                        </DialogDescription>
                      </DialogHeader>
                      <Textarea
                        placeholder={t("sponReq.responsePh")}
      ...
      <div>
        <h3 className="font-semibold text-foreground mb-4">{t("sponReq.history")}</h3>
        <div className="space-y-3">
          {requests.map((request) => (
            ...
            >
              <div>
                <p className="font-medium text-foreground">
                  {t("sponReq.offerAmt", { amount: request.bid_amount })}
                </p>
                <p className="text-sm text-muted-foreground">
                  {formatDate(request.created_at, { month: "short", day: "numeric", year: "numeric" })}
                </p>
              </div>
              <Badge className={statusColors[request.status || "pending"]}>
                {request.status}
