...
type DemandSignalState = "early" | "warming" | "unlocking" | "unlocked";
      ...
      <div className="grid min-h-[19rem] sm:grid-cols-[minmax(0,1fr)_104px]">
        <div className="flex min-w-0 flex-col p-6 sm:p-7">
          ...
          <div className="flex flex-1 flex-col justify-between py-5">
            <div>
              {matchedAsk ? (
                <p className="mb-2 text-xs font-semibold text-orange-800">People are asking for “{matchedAsk}”</p>
              ) : null}
              ...
              {leadingOption ? (
                <p className="mt-3 max-w-xl text-sm leading-6 text-[#5a493d]">
                  Current pull: <span className="font-bold text-[#1a120c]">{leadingOption}</span>
                </p>
              ) : null}
              {unlockLabel ? (
                <p className="mt-2 max-w-xl text-sm leading-6 text-[#5a493d]">
                  If enough people move, this can open: <span className="font-bold text-[#1a120c]">{unlockLabel}</span>
                </p>
              ) : null}
            </div>
              ...
              <div className="flex flex-wrap items-end justify-between gap-4">
                <div>
                  <div className="flex items-center gap-2 text-[#5a493d]">
                    ...
                    <span className="text-xs font-bold uppercase tracking-[0.12em]">People in</span>
                  </div>
                  <p className="mt-1 font-mono text-4xl font-black tracking-[-0.06em]">{demandCount.toLocaleString()}</p>
                </div>
                {threshold ? (
                  <div className="text-right">
                    <p className="font-mono text-sm font-black">{remaining === 0 ? "Threshold reached" : `${remaining.toLocaleString()} to open`}</p>
