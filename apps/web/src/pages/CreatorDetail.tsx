...
export default function CreatorDetail() {
  ...
  const momentIds = (momentsQuery.data || []).map((moment: any) => moment.id).filter(Boolean);
  const scenesQuery = useQuery({
    queryKey: ["public-creator-scenes", creator?.user_id, momentIds.join(",")],
    enabled: Boolean(creator?.user_id && momentIds.length),
    queryFn: async () => {
      ...
      const sceneIds = [...new Set((links || []).map((link: any) => link.scene_id).filter(Boolean))];
  ...
  const places=Array.from(new Map(moments.filter((moment:any)=>moment.venue_slug).map((moment:any)=>[moment.venue_slug,moment])).values()).slice(0,6) as any[];
  const hasWorldRelationships=places.length>0||scenes.length>0;
  ...
  return <main className="marketing-cinematic public-object-page min-h-screen bg-[#050505] pb-20 text-white">
    ...
    {hasWorldRelationships?<section className="border-t border-white/10 px-5 py-14 sm:px-6 md:py-20"><div className="mx-auto max-w-[1180px]"><div className="marketing-section-head"><div><p className="marketing-kicker">Around this creator</p><h2 className="mt-3 text-4xl font-black">Places and Scenes connected to their Moments.</h2></div></div><div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">{places.map((place:any)=><Link key={`place-${place.venue_slug}`} to={`/venues/${place.venue_slug}`} className="group border border-white/10 p-5 transition hover:border-orange-400/35"><p className="text-[9px] font-black uppercase tracking-[.14em] text-orange-300">Places they move</p><h3 className="mt-3 text-xl font-black">{place.venue_name||place.location||"Open place"}</h3><div className="mt-5 flex items-center justify-between text-xs text-white/42"><span>Connected through {place.title}</span><ArrowRight className="h-4 w-4 transition group-hover:translate-x-1 group-hover:text-orange-300"/></div></Link>)}{scenes.map((scene:any)=><Link key={`scene-${scene.id}`} to={`/scenes/${scene.slug}`} className="group border border-white/10 p-5 transition hover:border-orange-400/35"><p className="text-[9px] font-black uppercase tracking-[.14em] text-orange-300">Scenes they move</p><h3 className="mt-3 text-xl font-black">{scene.title}</h3><p className="mt-2 line-clamp-2 text-xs leading-5 text-white/42">{scene.description||"Open the Scene connected to this creator’s Moments."}</p><div className="mt-5 flex justify-end"><ArrowRight className="h-4 w-4 transition group-hover:translate-x-1 group-hover:text-orange-300"/></div></Link>)}</div></div></section>:null}
