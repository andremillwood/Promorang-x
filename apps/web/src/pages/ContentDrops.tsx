...
import { ArrowRight, BadgeCheck, Link2, Play, Plus, RadioTower } from "lucide-react";
      ...
      <div className="w-full px-4 py-6 sm:px-6 lg:px-8">
        <section className="rounded-3xl border border-primary/25 bg-gradient-to-br from-primary/10 via-black to-black p-6 sm:p-8">
          ...
          <div className="mt-6 grid max-w-2xl grid-cols-3 gap-2">
            {[
              { label: "Live releases", value: totals.active, icon: RadioTower },
              { label: "Assets", value: totals.assets, icon: Link2 },
              { label: "Linked Moments", value: totals.linked, icon: BadgeCheck },
            ].map((item) => (
              ...
              <CardContent>
                {!session?.access_token ? (
                  ...
                  <form onSubmit={submit} className="grid gap-4 lg:grid-cols-2">
                    ...
                    <div className="lg:col-span-2"><Button disabled={createDrop.isPending || addAsset.isPending} type="submit"><RadioTower className="mr-2 h-4 w-4" />Publish release</Button></div>
                  </form>
                )}
              </CardContent>
            </Card>
