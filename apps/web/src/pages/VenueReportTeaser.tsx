...
export default function VenueReportTeaser() {
    ...
    const { data: reportData, isLoading } = useQuery({
        ...
        queryFn: async () => {
            ...
            if (isDemo) {
                ...
                return {
                    venue_name: demoMoment?.venue_name || null,
                    location: demoMoment?.location || null,
                    participants: demoMoment?.participant_count || 45,
                    photos: Math.floor((demoMoment?.participant_count || 45) * 1.5),
                    moment_title: demoMoment?.title || null,
                };
            }
    ...
    const venueName = reportData?.venue_name || t("venueTease.fallbackVenue");
    const location = reportData?.location || t("venueTease.fallbackLocation");
    const momentTitle = reportData?.moment_title || t("venueTease.fallbackMoment");
    ...
    return (
        <div className="min-h-screen bg-background">
            <SEO title={t("venueTease.seoTitle", { name: venueName })} description={t("venueTease.seoDescription")} />
                ...
                <div className="container px-6 relative z-10">
                    <div className="max-w-3xl mx-auto text-center">
                        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/10 text-white mb-6">
                            ...
                            <span className="text-sm font-bold uppercase tracking-widest text-primary">{t("venueTease.badge")}</span>
                        </div>
                ...
                <div className="max-w-4xl mx-auto">
                    ...
                    <div className="bg-card rounded-[2rem] border border-border/60 shadow-xl overflow-hidden mb-8">
                        <div className="p-8 md:p-12 border-b border-border text-center">
                            <h2 className="text-2xl font-bold mb-2">{t("venueTease.activityTitle", { title: momentTitle })}</h2>
                            <p className="text-muted-foreground">
                                {t("venueTease.activityLede")}
                            </p>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-border">
                            ...
                            <div className="p-8 text-center relative group">
                                ...
                                <h3 className="text-sm font-bold text-muted-foreground uppercase tracking-widest mb-2">{t("venueTease.attendees")}</h3>
                            ...
                            <div className="p-8 text-center relative group">
                                ...
                                <h3 className="text-sm font-bold text-emerald-600/70 uppercase tracking-widest mb-2">{t("venueTease.captured")}</h3>
                            ...
                            <div className="p-8 text-center relative group">
                                ...
                                <h3 className="text-sm font-bold text-accent/70 uppercase tracking-widest mb-2">{t("venueTease.leads")}</h3>
                                <p className="text-5xl font-black text-accent blur-sm select-none">{t("venueTease.unlock")}</p>
                            </div>
                        </div>

                        {/* Neighborhood Benchmarking */}
                        <div className="p-8 md:p-12 border-t border-border bg-muted/20">
                            <h3 className="text-sm font-bold uppercase tracking-[0.2em] text-muted-foreground mb-8 text-center">{t("venueTease.benchmark")}</h3>
                            <div className="space-y-6 max-w-2xl mx-auto">
                                <div className="flex items-center gap-4">
                                    <div className="flex-1">
                                        <div className="flex justify-between mb-2">
                                            <span className="text-sm font-bold">{t("venueTease.you", { name: venueName })}</span>
                                            <span className="text-sm text-muted-foreground">{t("venueTease.managedNo")}</span>
                                        </div>
                                        <div className="h-4 w-full bg-secondary rounded-full overflow-hidden">
                                            <div className="h-full bg-primary/40" style={{ width: '40%' }} />
                                        </div>
                                    </div>
                                    <div className="w-20 text-right"><span className="text-xs font-bold text-muted-foreground">{t("venueTease.baseline")}</span></div>
                                </div>

                                <div className="flex items-center gap-4 opacity-70">
                                    <div className="flex-1">
                                        <div className="flex justify-between mb-2">
                                            <span className="text-sm font-bold blur-[4px]">{t("venueTease.peerA")}</span>
                                            <span className="text-[10px] font-black uppercase text-emerald-600 bg-emerald-500/10 px-1.5 py-0.5 rounded italic">{t("venueTease.subsidized")}</span>
                                        </div>
                                        <div className="h-4 w-full bg-secondary rounded-full overflow-hidden">
                                ...
                                <div className="flex items-center gap-4 opacity-70">
                                    <div className="flex-1">
                                        <div className="flex justify-between mb-2">
                                            <span className="text-sm font-bold blur-[4px]">{t("venueTease.peerB")}</span>
                                            <span className="text-[10px] font-black uppercase text-blue-600 bg-blue-500/10 px-1.5 py-0.5 rounded italic">{t("venueTease.agency")}</span>
                                        </div>
                                        <div className="h-4 w-full bg-secondary rounded-full overflow-hidden">
                        ...
                        <div className="bg-charcoal text-white p-8 md:p-12 text-center border-t border-border flex flex-col items-center justify-center gap-4 relative overflow-hidden">
                            ...
                            <Button variant="hero" size="xl" className="shadow-2xl h-16 px-10 text-lg group" asChild>
                                <Link to="/auth?role=merchant&intent=claim_venue">
                                    {t("venueTease.claimCta")}
                                    <ArrowRight className="w-5 h-5 ml-2 group-hover:translate-x-1 transition-transform" />
                                </Link>
                            </Button>
                            <p className="text-[10px] text-white/40 uppercase tracking-widest mt-4">{t("venueTease.noCard")}</p>
                        </div>
                    </div>
