...
export default function CommunityDetail() {
  ...
  const handleInvitePeople = async () => {
    ...
    try {
      ...
      toast({ title: "Invite ready", description: "Your people stay attributed to you." });
    } catch (error) {
      toast({ title: "Could not make an invite", description: (error as Error).message, variant: "destructive" });
    }
  };
  return (
    <main className="min-h-screen bg-black pb-24 text-white">
      ...
      <section className="relative min-h-[700px] overflow-hidden border-b border-white/10 pt-24">
        ...
        <div className="container relative flex min-h-[604px] items-end px-6 pb-12">
          <div className="grid w-full gap-10 lg:grid-cols-[1fr_360px] lg:items-end">
            ...
            <aside className="border-t border-white/20 pt-6 backdrop-blur-sm"><p className="text-[10px] font-black uppercase tracking-[.24em] text-primary">{state.eyebrow}</p><h2 className="mt-3 font-serif text-3xl font-bold leading-tight">{state.title}</h2><p className="mt-3 text-sm leading-6 text-white/55">{state.body}</p><p className="mt-4 text-xs text-white/50">{hub.data?.people ? `${hub.data.people.toLocaleString()} people` : ""}{hub.data?.activeThisWeek ? ` · ${hub.data.activeThisWeek.toLocaleString()} active this week` : ""}{hub.data?.operator?.name ? ` · ${hub.data.operator.name}` : ""}</p><div className="mt-6 flex gap-2">{membership?.membership_state === "active" && nextMoment ? <Link to={`/moments/${nextMoment.id}`} className="inline-flex min-h-12 flex-1 items-center justify-center gap-2 rounded-full bg-primary px-5 text-sm font-black text-black">{state.ctaLabel}<ArrowRight className="h-4 w-4"/></Link> : <button type="button" disabled={joinScene.isPending} onClick={handleJoin} className="inline-flex min-h-12 flex-1 items-center justify-center gap-2 rounded-full bg-primary px-5 text-sm font-black text-black disabled:opacity-60">{joinScene.isPending ? t("sceneDetail.joining") : nextMoment ? t("sceneDetail.join") : t("sceneDetail.keep")}<ArrowRight className="h-4 w-4"/></button>}<button type="button" aria-label={t("sceneDetail.share")} onClick={share} className="grid h-12 w-12 place-items-center rounded-full border border-white/20"><Share2 className="h-4 w-4"/></button></div></aside>
          </div>
        </div>
      </section>
      ...
      <section className="container px-6 pb-16">
        <p className="text-[10px] font-black uppercase tracking-[.28em] text-primary">Want to get involved?</p>
        <h2 className="mt-3 font-serif text-4xl font-bold">Contribute here. Keep the credit.</h2>
        <p className="mt-3 max-w-xl text-sm text-white/50">You do not need to own this community to bring people, drop a perk, or earn from what you move.</p>
        <div className="mt-6 grid gap-3 sm:grid-cols-2">
          <Link to={`/give?hub=${scene.id}`} className="rounded-[1.6rem] border border-white/10 px-5 py-5"><p className="font-serif text-2xl font-bold">Bring something</p><p className="mt-1 text-sm text-white/45">A perk, offer, night or experience.</p></Link>
          <Link to={`/create?hub=${scene.id}`} className="rounded-[1.6rem] border border-white/10 px-5 py-5"><p className="font-serif text-2xl font-bold">Create something</p><p className="mt-1 text-sm text-white/45">Get people to go, try, answer or show up.</p></Link>
          <Link to={`/earn?hub=${scene.id}`} className="rounded-[1.6rem] border border-white/10 px-5 py-5"><p className="font-serif text-2xl font-bold">Earn here</p><p className="mt-1 text-sm text-white/45">Opportunities already waiting in this room.</p></Link>
          <button type="button" onClick={handleInvitePeople} className="rounded-[1.6rem] border border-white/10 px-5 py-5 text-left"><p className="font-serif text-2xl font-bold">Invite my people</p><p className="mt-1 text-sm text-white/45">They stay attributed to you.</p></button>
        </div>
      </section>
      <MobileBottomNav />
    </main>
