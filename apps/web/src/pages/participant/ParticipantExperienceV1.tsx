...
function ParticipantNav({ active }: { active: Surface }) {
  return (
    <nav className="sticky bottom-0 z-30 grid grid-cols-6 border-t border-white/10 bg-[#080809]/95 px-2 pb-[max(.55rem,env(safe-area-inset-bottom))] pt-2.5 backdrop-blur-xl lg:rounded-b-[2rem]">
      ...
      <a href="/search" className="flex min-h-12 flex-col items-center justify-center gap-1 text-[11px] font-bold text-white/55 hover:text-[#ff6a00]" aria-label="Search all PROMORANG"><Search className="h-4 w-4" aria-hidden="true" />Search</a>
    </nav>
  );
}
...
function DiscoverSurface({ model }: { model: ReturnType<typeof useParticipantModel> }) {
  ...
  return (
    <Shell active="discover" model={model}>
      ...
      <SurfaceHeader kicker={`For you · ${model.cityName}`} title="Real opportunities. Near you." action={<a href="/search" className="grid h-10 w-10 place-items-center rounded-full border border-white/12 bg-white/[0.04] transition hover:border-[#ff6a00]/60 hover:text-[#ff9a4d]" aria-label="Search all PROMORANG"><Search className="h-4 w-4" /></a>} />
