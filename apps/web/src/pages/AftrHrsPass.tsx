...
export default function AftrHrsPass() {
  ...
  const share = async () => {
    ...
    if (navigator.share) await navigator.share({ title: t("aftrhrs.passShareTitle"), url });
  ...
  return (
    <main className="min-h-screen bg-black px-4 py-16 text-white">
      <SEO
        title={t("aftrhrs.passSeo")}
        description={t("aftrhrs.confirmation")}
      ...
      <div className="mx-auto max-w-md">
        <p className="text-[11px] font-black uppercase tracking-[0.24em] text-cyan-300">AftrHrs · {t("aftrhrs.when")}</p>
        <h1 className="mt-3 text-4xl font-black uppercase tracking-[-0.05em]">{t("aftrhrs.passTitle")}</h1>
        {!user ? (
          <div className="mt-8 rounded-[2rem] border border-white/10 bg-white/[0.04] p-6">
            <p className="text-white/70">
              {t("aftrhrs.passSignInCopy")}
            </p>
            <p className="mt-4 text-sm font-bold uppercase tracking-[0.14em] text-fuchsia-300">{t("aftrhrs.arrivalRule")}</p>
            <button
              type="button"
              onClick={openSignIn}
              className="mt-6 w-full rounded-full bg-white px-6 py-3 text-sm font-black uppercase tracking-[0.16em] text-black"
            >
              {t("aftrhrs.passSignInCta")}
            </button>
            <Link
          ...
          <article className="mt-8 overflow-hidden rounded-[2rem] border border-white/10 bg-white/[0.04]">
            ...
            <div className="p-6">
              <p className="text-xs uppercase tracking-[0.2em] text-white/45">{t("aftrhrs.digitalWhen", { when: t("aftrhrs.when") })}</p>
              ...
              <p className="mt-5 text-sm leading-6 text-white/70">{t("aftrhrs.confirmation")}</p>
              <p className="mt-3 text-sm font-bold uppercase tracking-[0.14em] text-fuchsia-300">{t("aftrhrs.arrivalRule")}</p>
              <p className="mt-3 text-xs uppercase tracking-[0.16em] text-white/40">{localizedGuestPassStatus(pass.status, t)}</p>
        ...
        <div className="mt-8 flex flex-col items-center gap-4">
          <div className="flex flex-wrap justify-center gap-4">
            <Link to={AFTRHRS_PATHS.landing} className="text-sm uppercase tracking-[0.16em] text-white/50">{t("aftrhrs.backEvent")}</Link>
            {user ? (
              <Link to="/wallet" className="text-sm uppercase tracking-[0.16em] text-cyan-300">{t("aftrhrs.walletLink")}</Link>
            ) : null}
          </div>
          <div className="flex flex-col items-center gap-2">
            <img src={promorangLogo} alt="PROMORANG" className="h-7 w-auto object-contain" />
            <p className="text-[10px] font-black uppercase tracking-[0.28em] text-white/55">{t("aftrhrs.powered")}</p>
          </div>
        </div>
      </div>
    </main>
