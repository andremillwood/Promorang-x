...
export function PiecePortfolio() {
  ...
  const fetchPortfolio = useCallback(async () => {
    if (!session?.access_token) { setPositions([]); setLoading(false); return; }
...
function Summary({ label, value, detail, positive }: { label: string; value: string; detail: string; positive?: boolean }) { return <div className="bg-[#101012] p-5 sm:p-6"><p className="text-[10px] font-black uppercase tracking-[.18em] text-white/38">{label}</p><p className={`mt-2 text-2xl font-black tracking-[-.04em] ${positive ? "text-emerald-300" : "text-white"}`}>{value}</p><p className="mt-1 text-[11px] text-white/36">{detail}</p></div>; }
function State({ icon: Icon, title, copy, action, href, onAction }: { icon: typeof Layers3; title: string; copy: string; action: string; href?: string; onAction?: () => void }) { return <section className="mx-auto max-w-2xl rounded-[32px] border border-dashed border-white/15 bg-white/[.025] px-6 py-16 text-center"><div className="mx-auto grid h-14 w-14 place-items-center rounded-2xl border border-primary/20 bg-primary/10 text-primary"><Icon className="h-6 w-6" /></div><h2 className="mt-5 text-2xl font-black tracking-[-.035em]">{title}</h2><p className="mx-auto mt-3 max-w-lg text-sm leading-6 text-white/48">{copy}</p>{href ? <Button asChild className="mt-6 rounded-xl"><Link to={href}>{action}<ArrowRight className="ml-2 h-4 w-4" /></Link></Button> : <Button onClick={onAction} className="mt-6 rounded-xl">{action}<ArrowRight className="ml-2 h-4 w-4" /></Button>}</section>; }
