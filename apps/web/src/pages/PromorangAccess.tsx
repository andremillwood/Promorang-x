...
const filters = ["All", "Tonight", "Music", "VIP", "Tables", "Secret"];
...
export default function PromorangAccess() {
  ...
  const claim = async (experience: PresentsExperience) => {
    ...
    try {
      ...
      toast.success(result.status === "pending" ? "Claim sent for host approval" : "Access claimed");
    } catch (error) { toast.error(error instanceof Error ? error.message : "Claim failed"); }
    ...
    <main className="min-h-screen bg-[#11100e] text-[#f4efe5]">
      <section className="relative overflow-hidden border-b border-white/10 px-5 pb-16 pt-28 sm:px-10 lg:px-20 lg:pb-24">
        ...
        <div className="relative mx-auto max-w-7xl">
          <p className="mb-5 flex items-center gap-2 text-xs font-bold tracking-[.24em] text-orange-400"><LockKeyhole className="h-4 w-4" /> PROMORANG ACCESS</p>
          <div className="grid items-end gap-8 lg:grid-cols-[1fr_.5fr]">
            <div><h1 className="max-w-4xl font-serif text-5xl leading-[.92] tracking-[-.045em] sm:text-7xl lg:text-8xl">Access money can’t always buy.</h1><p className="mt-6 max-w-2xl text-lg leading-8 text-stone-400">Complete the challenge. Bring the right people. Unlock the experience.</p></div>
            <div className="border-l border-orange-500 pl-5 text-sm leading-6 text-stone-400"><strong className="block text-2xl text-white">PromoKeys open doors.</strong>Every unlock has clear requirements, live inventory and a credential staff can verify.</div>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-5 py-12 sm:px-10 lg:px-20">
        <div className="mb-10 flex gap-2 overflow-x-auto pb-2" aria-label="Filter experiences">
          {filters.map((item) => <button key={item} onClick={() => setFilter(item)} className={`whitespace-nowrap border px-4 py-2 text-xs font-bold uppercase tracking-wider ${filter === item ? 'border-orange-500 bg-orange-500 text-white' : 'border-white/15 text-stone-400 hover:border-white/40'}`}>{item}</button>)}
        </div>
            ...
            {visible.map((experience, index) => {
              ...
              return <article key={experience.id} className="group relative overflow-hidden border border-white/10 bg-[#181613] p-6 sm:p-8">
                ...
                <div className="relative">
                  ...
                  <div className="mt-7 flex flex-wrap gap-4 text-xs text-stone-400"><span className="flex gap-1.5"><Clock3 className="h-4 w-4" />{experience.metadata?.day || "This week"}</span><span className="flex gap-1.5"><MapPin className="h-4 w-4" />{experience.venue_name}</span><span className="flex gap-1.5"><TicketCheck className="h-4 w-4" />{remaining} remaining</span></div>
                  <div className="mt-7 grid gap-2 border-y border-white/10 py-5 text-sm sm:grid-cols-3">
                    <span className="flex items-center gap-2"><KeyRound className="h-4 w-4 text-orange-400" />{experience.promo_keys_required || "No"} Keys</span>
                    <span className="flex items-center gap-2"><Users className="h-4 w-4 text-orange-400" />{experience.referrals_required} referrals</span>
                    <span className="flex items-center gap-2"><Sparkles className="h-4 w-4 text-orange-400" />{experience.mission_requirements.length} missions</span>
                  </div>
                  {credential ? <div className="mt-6 border border-emerald-500/40 bg-emerald-500/10 p-4"><span className="flex items-center gap-2 text-xs font-bold text-emerald-400"><Check className="h-4 w-4" /> CLAIMED</span><code className="mt-2 block text-xl font-bold tracking-widest">{credential}</code></div> : <button disabled={remaining === 0 || claiming === experience.id} onClick={() => claim(experience)} className="mt-6 flex w-full items-center justify-center gap-2 bg-orange-500 px-5 py-4 text-sm font-black text-white hover:bg-orange-400 disabled:opacity-40">{remaining === 0 ? "Fully claimed" : claiming === experience.id ? "Checking eligibility…" : "Unlock this experience"}<ArrowRight className="h-4 w-4" /></button>}
                </div>
              </article>;
            })}
          </div>
        )}
        {!membership && <div className="mt-12 flex flex-col items-start justify-between gap-5 border border-orange-500/30 bg-orange-500/10 p-6 sm:flex-row sm:items-center"><div><strong className="font-serif text-2xl">The list is invitation-only.</strong><p className="mt-1 text-sm text-stone-400">Enter through Promorang Presents before claiming access.</p></div><Link to="/presents#access" className="flex items-center gap-2 bg-white px-5 py-3 text-sm font-bold text-black">Enter a code <ArrowRight className="h-4 w-4" /></Link></div>}
      </section>
    </main>
  );
}
