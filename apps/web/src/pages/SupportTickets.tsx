...
export default function SupportTickets() {
  ...
  async function fetchTickets() {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(payload.error || t("support.loadFailed"));
      }
      setTickets(Array.isArray(payload) ? payload : []);
    } catch (fetchError) {
      setError(fetchError instanceof Error ? fetchError.message : t("support.loadFailed"));
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      ...
      if (!response.ok || payload.success === false) {
        throw new Error(payload.error || t("support.submitFailed"));
      }

      toast({
        title: t("support.submittedTitle"),
          ...
          <section className="grid gap-6 lg:grid-cols-[1.05fr_0.95fr]">
            <div className="border border-border bg-card rounded-xl p-6">
              <div className="mb-6 flex items-center justify-between gap-4">
                <div>
                  <h2 className="text-xl font-semibold">{t("support.queue")}</h2>
              ...
              {loading ? (
                ...
                <div className="flex min-h-[240px] flex-col items-center justify-center text-center">
                  ...
                  <p className="font-medium">{t("support.emptyTitle")}</p>
                    ...
                    >
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        ...
                        <span className={`rounded-full border px-2.5 py-1 text-xs font-medium ${statusTone[ticket.status] || "bg-muted text-foreground border-border"}`}>
                          {t((ticket.status === "in_progress" ? "support.statusInProgress" : ticket.status === "resolved" ? "support.statusResolved" : ticket.status === "closed" ? "support.statusClosed" : "support.statusOpen"))}
                        </span>
                      </div>
                    </Link>
                  ))}
            ...
            <div className="border border-border bg-card rounded-xl p-6">
              <div className="mb-6 flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-xl font-semibold">{receiptId ? t("support.commerceTitle") : t("support.newTitle")}</h2>
                  <p className="text-sm text-muted-foreground">{receiptId ? t("support.commerceCopy", { product: receiptProduct || t("support.yourReceipt") }) : t("support.newCopy")}</p>
                </div>
              </div>

              <form onSubmit={handleSubmit} className="space-y-4">
                {receiptId ? <div className="space-y-2"><label htmlFor="reason" className="text-sm font-medium">{t("support.whatWrong")}</label><select id="reason" value={form.reason} onChange={(event)=>setForm((current)=>({...current,reason:event.target.value}))} className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm"><option value="reward_not_honoured">{t("support.reasonReward")}</option><option value="code_failed">{t("support.reasonCode")}</option><option value="merchant_closed">{t("support.reasonClosed")}</option><option value="offer_differed">{t("support.reasonDiffered")}</option><option value="purchase_problem">{t("support.reasonPurchase")}</option><option value="other">{t("support.reasonOther")}</option></select><p className="text-xs text-muted-foreground">{t("support.receiptHint", { id: receiptId.slice(0,8) })}</p></div> : null}
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <div className="space-y-2">
                    ...
                    <select
                      ...
                      className="h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
                    >
                      <option value="low">{t("support.priLow")}</option>
                      <option value="medium">{t("support.priMedium")}</option>
                      <option value="high">{t("support.priHigh")}</option>
                    </select>
                  </div>
                </div>
