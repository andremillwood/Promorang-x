...
export default function CommentSystem({ contentId, currentUser, isOwner = false }: CommentSystemProps) {
  ...
  const CommentItem = ({ comment, isReply = false }: { comment: Comment; isReply?: boolean }) => {
    ...
    return (
      <div className={`${isReply ? 'ml-8 border-l-2 border-pr-surface-3 pl-4' : ''} mb-4`}>
        <div className="bg-pr-surface-2 rounded-lg p-4">
          ...
          <div className="flex items-start justify-between mb-2">
            <div className="flex items-center space-x-2">
              <UserLink
                ...
                size="sm"
              />
              <span className="text-xs text-pr-text-2">{formatTimestamp(comment.created_at)}</span>
              {comment.is_pinned && (
            ...
            {(isAuthor || isOwner) && (
              <div className="relative group">
                <button className="text-gray-400 hover:text-pr-text-2 p-1">
                  <MoreVertical className="w-4 h-4" />
                </button>
                <div className="absolute right-0 top-full bg-pr-surface-card border border-pr-surface-3 rounded-lg shadow-lg z-10 min-w-32 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all">
                  {isOwner && !comment.is_pinned && (
                    <button
                      onClick={() => pinComment(comment.id)}
                      className="w-full text-left px-3 py-2 text-sm hover:bg-pr-surface-2 flex items-center space-x-2"
                    >
                      <Pin className="w-3 h-3" />
                      <span>Pin</span>
                    </button>
                  ...
                  {(isAuthor || isOwner) && (
                    <button
                      ...
                      className="w-full text-left px-3 py-2 text-sm hover:bg-pr-surface-2 text-red-600 flex items-center space-x-2"
                    >
          ...
          {/* Comment Text */}
          <p className="text-pr-text-1 mb-3 whitespace-pre-wrap">{comment.comment_text}</p>
          ...
          <div className="flex items-center space-x-4">
            <button
              ...
              className={`flex items-center space-x-1 text-sm transition-colors ${
                comment.has_liked 
                  ? 'text-red-500' 
                  : 'text-pr-text-2 hover:text-red-500'
            ...
            {!isReply && currentUser && (
              <button
                ...
                className="flex items-center space-x-1 text-sm text-pr-text-2 hover:text-blue-500 transition-colors"
              >
            ...
            <div className="mt-3 pt-3 border-t border-pr-surface-3">
              <div className="flex space-x-2">
                <textarea
                  ...
                  className="flex-1 p-2 border border-pr-surface-3 rounded-lg text-sm resize-none"
                  rows={2}
                />
                <div className="flex flex-col space-y-1">
                  ...
                  <button
                    ...
                    className="text-pr-text-2 hover:text-pr-text-1 p-2"
                  >
                    ✕
                  </button>
                </div>
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="flex items-center space-x-3 pb-4 border-b border-pr-surface-3">
      ...
      {currentUser ? (
          ...
          <div className="flex items-start space-x-3">
            ...
            <div className="flex-1">
              <textarea
                ...
                className="w-full p-3 border border-pr-surface-3 rounded-lg resize-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                rows={3}
              />
        ...
        <div className="bg-pr-surface-2 border border-pr-surface-3 rounded-lg p-4 text-center">
          <p className="text-pr-text-2 mb-2">Sign in to join the conversation</p>
          <button 
      ...
      <div className="space-y-4">
        {comments.length > 0 ? (
          ...
          <div className="text-center py-12">
            ...
            <h4 className="text-lg font-medium text-pr-text-1 mb-2">No comments yet</h4>
