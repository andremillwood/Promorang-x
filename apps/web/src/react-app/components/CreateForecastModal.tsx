        ...
        <div className="border-b border-pr-surface-3 px-6 pt-6 pb-4">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              aria-label="Close create forecast modal"
            >
              <X className="h-5 w-5" />
            </button>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 pb-6">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <div className="space-y-3">
                ...
                {contentOptions.length > 0 && (
                  <div>
                    <p className="text-xs text-pr-text-2 mb-2">Select from Promorang content:</p>
                    <div className="max-h-32 overflow-y-auto border border-pr-surface-3 rounded-lg">
                      {contentOptions.map((content) => (
                        <button
                          key={content.id}
                          type="button"
                          onClick={() => handleContentSelect(content)}
                          className={`w-full text-left p-3 border-b border-pr-border hover:bg-pr-surface-2 ${
                            formData.content_id === content.id.toString() ? 'bg-blue-50 border-l-4 border-l-blue-500' : ''
                          }`}
                        >
                          <div className="text-sm font-medium text-pr-text-1">{content.title}</div>
                          <div className="text-xs text-pr-text-2">{content.platform} • by {content.creator_name}</div>
                        </button>
                      ))}
                    </div>
                  </div>
                ...
                {/* Manual URL Entry */}
                <div>
                  <p className="text-xs text-pr-text-2 mb-2">Or enter external content URL:</p>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <select
                      value={formData.platform}
                      onChange={(e) => setFormData({ ...formData, platform: e.target.value })}
                      className="px-3 py-2 border border-pr-surface-3 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                    >
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              <h4 className="font-medium text-pr-text-1 mb-3">Your Initial Prediction & Stake</h4>
              ...
              {formData.initial_amount && (
                <div className="mt-3 p-3 bg-pr-surface-card rounded-lg border border-blue-200">
