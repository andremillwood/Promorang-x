...
export default function CouponCard({ coupon, compact = false }: CouponCardProps) {
  ...
  if (compact) {
    return (
      <div
        onClick={handleClick}
        className="bg-pr-surface-card rounded-lg border border-pr-surface-3 p-4 hover:shadow-md transition-all cursor-pointer group"
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3 flex-1">
            ...
            <div className="flex-1 min-w-0">
              <h4 className="font-semibold text-pr-text-1 truncate">{coupon.title}</h4>
              <p className="text-sm text-pr-text-2">
                {rewardsService.formatCouponValue(coupon)}
              </p>
            </div>
          </div>
          <ArrowRight className="h-5 w-5 text-gray-400 group-hover:text-pr-text-2 transition-colors flex-shrink-0" />
        </div>
      </div>
    );
  }
    ...
    >
      ...
      <div className={`p-4 bg-gradient-to-r ${getGradientClass()}`}>
        <div className="flex items-start justify-between text-white">
          ...
          {!coupon.is_redeemed && (
            <span className="px-2 py-1 rounded-full text-xs font-medium bg-pr-surface-card/20 backdrop-blur-sm">
              New
            </span>
          )}
        </div>
      ...
      <div className="p-6 space-y-4">
        <div>
          <h3 className="text-lg font-semibold text-pr-text-1 mb-1">{coupon.title}</h3>
          {coupon.description && (
            <p className="text-sm text-pr-text-2 line-clamp-2">{coupon.description}</p>
          )}
        </div>
