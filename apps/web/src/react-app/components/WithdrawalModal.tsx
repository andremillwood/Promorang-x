...
export default function WithdrawalModal({ user, isOpen, onClose, onSuccess }: WithdrawalModalProps) {
  ...
  const feePercentage = typeof selectedMethodData?.fee_percentage === 'number' ? selectedMethodData.fee_percentage : 0;
  ...
  const getRequiredFields = (method: string): string[] => {
    ...
    try {
      if (typeof methodData.required_fields === 'string') {
    ...
    >
      <div className="flex flex-col gap-6">
        <div className="flex items-center justify-between border-b border-pr-surface-3 pb-4">
          <h2 className="text-xl font-bold text-pr-text-1">Withdraw Gems</h2>
          <button
            onClick={handleClose}
        ...
        {!eligibleForWithdrawal ? (
          <div className="space-y-4 text-center">
            ...
            <h3 className="text-lg font-semibold text-pr-text-1">Insufficient Gems</h3>
            <p className="text-pr-text-2">You need at least 200 gems to request a withdrawal.</p>
            <p className="text-sm text-pr-text-2">Current balance: {user.gems_balance || 0} gems</p>
            <button
              onClick={handleClose}
              className="mt-2 rounded-lg bg-pr-surface-20 px-6 py-2 font-medium text-white transition-colors hover:bg-gray-600"
            >
              Close
            </button>
          </div>
          ...
          <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    <input
                      ...
                      className="w-full px-4 py-3 border border-pr-surface-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500"
                    />
                    <div className="flex space-x-2">
                      {[200, 500, 1000, user.gems_balance || 0].filter((v, i, arr) => arr.indexOf(v) === i && v >= 200).map((preset) => (
                        <button
                          ...
                          className="px-3 py-1 text-sm bg-pr-surface-2 hover:bg-pr-surface-3 rounded-full text-pr-text-1"
                        >
                    ...
                    <div className="mb-4">
                      ...
                      <div className="grid grid-cols-1 gap-2">
                        {userPreferences.map((pref) => {
                          ...
                          let displayInfo = '';
                          try {
                            const data = pref.preference_data ? JSON.parse(pref.preference_data) : {};
                          ...
                          return (
                            <button
                              ...
                              onClick={() => {
                                ...
                                try {
                                  const parsedData = pref.preference_data ? JSON.parse(pref.preference_data) : {};
                            ...
                            >
                              <div className="flex items-center space-x-3">
                                ...
                                <div>
                                  ...
                                  <p className="text-sm text-pr-text-2">{displayInfo}</p>
                                </div>
                                {pref.is_default && (
                      ...
                      <div className="relative my-4">
                        ...
                        <div className="relative flex justify-center text-sm">
                          <span className="px-2 bg-pr-surface-card text-pr-text-2">Or choose a new method</span>
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {withdrawalMethods.filter(method => method.is_active).map((method) => (
                      ...
                      >
                        ...
                        {typeof method.min_amount === 'number' && (
                          <p className="text-xs text-pr-text-2">Min: {method.min_amount} gems</p>
                        )}
                      </button>
                    ))}
                  </div>
            ...
            {step === 'details' && selectedMethodData && (
              <div className="space-y-4">
                <div>
                  <h3 className="text-lg font-medium text-pr-text-1 mb-1">{selectedMethodData.display_name}</h3>
                  {selectedMethodData.processing_time && (
                    <p className="mb-4 text-sm text-pr-text-2">Processing time: {selectedMethodData.processing_time}</p>
                  )}
                </div>
                    ...
                    <div key={field}>
                      ...
                      {field === 'network' && selectedMethod.startsWith('crypto_') ? (
                        <select
                          ...
                          className="w-full px-3 py-2 border border-pr-surface-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500"
                ...
                <div className="flex items-center space-x-2">
                  <input
                    ...
                    className="w-4 h-4 text-purple-600 bg-pr-surface-2 border-pr-surface-3 rounded focus:ring-purple-500"
                  />
                  <label htmlFor="saveAsDefault" className="text-sm text-pr-text-1">
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <div className="bg-pr-surface-2 rounded-lg p-4 space-y-3">
                    ...
                    {typeof selectedMethodData.fee_percentage === 'number' && (
                      <div className="flex justify-between">
                        <span className="text-pr-text-2">Processing Fee ({selectedMethodData.fee_percentage}%):</span>
                  ...
                  <div className="mt-4 p-4 border border-pr-surface-3 rounded-lg">
                    <h4 className="font-medium text-pr-text-1 mb-2">Payment Method: {selectedMethodData.display_name}</h4>
                ...
                <div className="flex space-x-3">
                  <button
                    onClick={() => setStep('details')}
                    disabled={loading}
                    className="flex-1 px-4 py-2 border border-pr-surface-3 rounded-lg text-pr-text-1 font-medium hover:bg-pr-surface-2 transition-colors disabled:opacity-50"
                  >
                    Back
                  </button>
                  <button
