...
export function KPICard({ title, value, change, changeType, icon, trend }: KPICardProps) {
  ...
  return (
    <div className="bg-pr-surface-card rounded-xl p-6 border border-pr-surface-3 shadow-sm min-w-0">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center space-x-2">
          {icon && <div className="text-pr-text-2">{icon}</div>}
          <h3 className="text-sm font-medium text-pr-text-2">{title}</h3>
        </div>
        {change !== undefined && (
      ...
      <div className="mb-4">
        <p className="text-2xl font-bold text-pr-text-1">{value}</p>
      </div>
