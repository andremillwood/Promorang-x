...
export default function EventCard({ event, compact = false }: EventCardProps) {
    ...
    if (compact) {
        return (
            <Link
                to={`/events/${event.id}`}
                className="flex gap-4 p-4 bg-pr-surface-card border border-pr-border rounded-xl hover:border-purple-500/50 transition-all group"
            >
    ...
    return (
        <Link
            to={`/events/${event.id}`}
            className="block bg-pr-surface-card border border-pr-border rounded-2xl overflow-hidden hover:border-purple-500/50 hover:shadow-xl transition-all group"
        >
            {/* Cover Image */}
