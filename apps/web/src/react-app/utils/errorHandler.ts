...
const getErrorResponseMessage = (error: unknown): string | undefined => {
  if (error && typeof error === 'object') {
    const err = error as ErrorLike;
...
function shouldRedirectToErrorPage(error: unknown): boolean {
...
export function handleApiError(
  ...
  const message = getErrorResponseMessage(error) ?? toErrorMessage(error);
  ...
  const isCritical = 
    (typeof status === 'number' && criticalStatuses.includes(status)) ||
    criticalMessages.some(msg => message.toLowerCase().includes(msg));
  
  if (isCritical) {
    handleCriticalError(
      new Error(`API Error (${status ?? 'unknown'}): ${message}`),
      component,
      action,
      userId
    );
...
export function createErrorHandler(component: string) {
  return {
    ...
    handleApiError: (error: unknown, action: string, userId?: string) => {
