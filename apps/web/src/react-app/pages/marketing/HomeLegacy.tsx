...
// Unlock Banner - Shows this is a preview of what consistent users see
const UnlockBanner = () => (
    ...
    <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-2">
      <div className="flex items-center gap-3">
        ...
        <span className="font-semibold text-sm">
          Full Platform Preview — This is what Day 14+ active users see
        </span>
      </div>
      <Link
        to="/auth"
        className="text-sm font-bold bg-white/20 hover:bg-white/30 px-4 py-1.5 rounded-full transition-colors flex items-center gap-2"
      >
...
const HeroSection = ({ activeStats, user, navigate, handleDemoLogin, demoLoginState, activePersona, onPersonaChange }: any) => {
  // Access Rank messaging - focus on consistency and priority access
  const personaCopy = {
    creator: {
      title: "Show up consistently.",
      highlight: "Get access others don't.",
      sub: "Promorang tracks active users and gives them priority when opportunities appear."
    },
    investor: {
      title: "Show up consistently.",
      highlight: "Get access others don't.",
      sub: "Build your Access Rank through consistent participation. Higher rank means earlier access."
    },
    merchant: {
      title: "Reach reliable users.",
      highlight: "Guaranteed actions.",
      sub: "Connect with verified, active users who complete what they start."
    },
    shopper: {
      title: "Show up consistently.",
      highlight: "Get access others don't.",
      sub: "Active users unlock opportunities that others miss. Your consistency is your advantage."
    }
  };
...
// Social Shield Highlight Section - Insurance/Protection Feature
const SocialShieldSection = ({ navigate }: { navigate: (path: string) => void }) => {
  return (
    <section className="py-24 bg-gradient-to-br from-emerald-900/20 via-pr-surface-background to-teal-900/20 relative overflow-hidden">
      {/* Background glow */}
      <div className="absolute inset-0 z-0">
        <div className="absolute top-[-20%] right-[-10%] w-[40vw] h-[40vw] bg-emerald-600/10 rounded-full blur-[100px]" />
        <div className="absolute bottom-[-20%] left-[-10%] w-[40vw] h-[40vw] bg-teal-600/10 rounded-full blur-[100px]" />
      </div>
        ...
        <div className="grid md:grid-cols-2 gap-12 items-center">
          ...
          <div>
            <div className="inline-flex items-center gap-2 bg-emerald-500/20 border border-emerald-500/30 px-4 py-2 rounded-full mb-6">
              ...
              <span className="text-sm font-bold text-emerald-400">SOCIAL MEDIA INSURANCE</span>
            </div>

            <h2 className="text-3xl md:text-5xl font-bold text-pr-text-1 mb-6 leading-tight">
              Never Lose Money to{' '}
            ...
            <p className="text-xl text-pr-text-2 mb-8 leading-relaxed">
              <strong className="text-emerald-400">Social Shield</strong> is your safety net. When your content underperforms due to algorithm changes, platform issues, or bad timing — we cover the difference.
            </p>

            <div className="space-y-4 mb-8">
              {[
                { text: "Automatic protection — no claims to file", icon: CheckCircle },
                { text: "Covers algorithm changes, strikes & bans", icon: Shield },
                { text: "Up to 100% coverage at Partner tier", icon: TrendingUp },
                { text: "Funded by your staking contributions", icon: Sparkles }
              ].map((item, i) => (
                <div key={i} className="flex items-center gap-3">
                  <div className="w-8 h-8 bg-emerald-500/20 rounded-lg flex items-center justify-center flex-shrink-0">
                    <item.icon className="w-4 h-4 text-emerald-500" />
                  </div>
                  <span className="text-pr-text-1 font-medium">{item.text}</span>
                </div>
              ))}
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <button
                ...
                className="px-8 py-4 bg-gradient-to-r from-emerald-500 to-teal-600 text-white rounded-xl font-bold hover:opacity-90 transition-all shadow-xl shadow-emerald-500/20 flex items-center justify-center gap-2"
              >
                Learn About Social Shield <Shield className="w-5 h-5" />
              </button>
              <button
                onClick={() => navigate('/auth')}
                className="px-8 py-4 bg-pr-surface-card border border-emerald-500/30 text-pr-text-1 rounded-xl font-bold hover:bg-pr-surface-2 transition-all flex items-center justify-center"
              >
                Get Protected Now
              </button>
            </div>
          ...
          {/* Right: Visual Demo */}
          <div className="bg-pr-surface-card border border-emerald-500/30 rounded-2xl p-8 shadow-2xl">
            <div className="text-center mb-6">
              <div className="w-16 h-16 bg-emerald-500/20 rounded-2xl flex items-center justify-center mx-auto mb-4">
                <Shield className="w-8 h-8 text-emerald-500" />
              </div>
              <h3 className="text-xl font-bold text-pr-text-1 mb-2">How Social Shield Works</h3>
              <p className="text-sm text-pr-text-2">Real protection for real creators</p>
            </div>

            <div className="space-y-4 mb-6">
              <div className="flex items-center justify-between p-4 bg-pr-surface-2 rounded-lg border border-pr-border">
                <span className="text-pr-text-2">Expected earnings</span>
                <span className="font-bold text-pr-text-1 text-lg">500 Gems</span>
              </div>
              <div className="flex items-center justify-between p-4 bg-red-500/10 rounded-lg border border-red-500/30">
                <div className="flex items-center gap-2">
                  <span className="text-red-400">⚠️ Algorithm change</span>
                </div>
                <span className="font-bold text-red-400 text-lg">-200 Gems</span>
              </div>
              <div className="flex items-center justify-between p-4 bg-emerald-500/10 rounded-lg border border-emerald-500/30">
                ...
                <span className="font-bold text-emerald-400 text-lg">+200 Gems</span>
              </div>
            </div>

            <div className="text-center p-4 bg-gradient-to-r from-emerald-500/20 to-teal-500/20 rounded-xl border border-emerald-500/30">
              <div className="text-3xl font-extrabold text-emerald-500">500 Gems</div>
              <div className="text-sm text-pr-text-2">Your earnings — protected, always</div>
            </div>

            <p className="text-xs text-center text-pr-text-muted mt-4">
              Available to all Promorang members. Higher tiers = more coverage.
            </p>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen-dynamic bg-pr-surface-background font-sans selection:bg-blue-500/30">
      <SEO
        ...
        keywords="access rank, consistent users, priority access, promorang, active users, opportunities"
      ...
      <SocialShieldSection navigate={navigate} />

      {/* EarningsCalculator removed per Access Rank refactor - available in /catalog */}
