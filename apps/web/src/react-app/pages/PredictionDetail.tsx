  ...
  if (!predictionId) {
    return (
      <div className="p-6">
        ...
        <p className="mt-6 text-pr-text-2">Prediction not found.</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-pr-surface-card rounded-2xl border border-pr-surface-3 shadow-sm p-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          <div>
            <h1 className="text-2xl font-semibold text-pr-text-1">{prediction.content_title}</h1>
            <p className="text-sm text-pr-text-2">
              {prediction.platform.toUpperCase()} • Placed {new Date(prediction.created_at).toLocaleDateString()}
            </p>
            <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-4">
              <div>
                <p className="text-xs text-pr-text-2 uppercase tracking-wide">Side</p>
                <p className="text-base font-semibold text-pr-text-1 capitalize">{prediction.prediction_side}</p>
              </div>
              <div>
                <p className="text-xs text-pr-text-2 uppercase tracking-wide">Stake</p>
                <p className="text-base font-semibold text-pr-text-1">${prediction.amount.toFixed(2)}</p>
              </div>
              <div>
                <p className="text-xs text-pr-text-2 uppercase tracking-wide">Potential payout</p>
                <p className="text-base font-semibold text-pr-text-1">${prediction.potential_payout.toFixed(2)}</p>
              </div>
              <div>
                <p className="text-xs text-pr-text-2 uppercase tracking-wide">Status</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-pr-surface-card border border-pr-surface-3 rounded-2xl p-6 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-pr-text-1">Trajectory</h2>
        ...
        <div className="bg-pr-surface-card border border-pr-surface-3 rounded-2xl p-6 shadow-sm">
          <h2 className="text-lg font-semibold text-pr-text-1 mb-4">Projection notes</h2>
          <p className="text-sm text-pr-text-2 leading-relaxed">{forecast.creator_projection}</p>
        </div>
      </div>
      ...
      <div className="bg-pr-surface-card border border-pr-surface-3 rounded-2xl p-6 shadow-sm">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-2">
            ...
            <h2 className="text-lg font-semibold text-pr-text-1">Participants</h2>
          </div>
        ...
        <div className="space-y-3">
          {participants.map((participant, index) => (
            <div key={`${participant.username}-${index}`} className="flex items-center justify-between bg-pr-surface-2 rounded-lg px-4 py-3">
              <div>
                <p className="font-medium text-pr-text-1">{participant.username}</p>
                <p className="text-xs text-pr-text-2 capitalize">{participant.side}</p>
              </div>
              <div className="text-right text-sm text-pr-text-2">
                <p>${participant.amount.toFixed(2)}</p>
                <p className="text-xs text-pr-text-2 capitalize">{participant.status}</p>
              </div>
            </div>
          ))}
        </div>
