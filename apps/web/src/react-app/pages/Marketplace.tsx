  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-pr-text-1">Task Marketplace</h1>
          <p className="text-pr-text-2 mt-2">Discover earning opportunities that match your skills</p>
        </div>
        <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-11 pr-4 py-2 bg-pr-surface-1 border border-pr-surface-3 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent text-pr-text-1 placeholder:text-gray-400"
            />
          </div>
          ...
          <select
            ...
            className="px-4 py-2 border border-pr-surface-3 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent"
          >
          ...
          <button className="flex items-center justify-center space-x-2 px-4 py-2 border border-pr-surface-3 rounded-lg hover:bg-pr-surface-2 transition-colors">
            <Filter className="w-5 h-5 text-pr-text-2" />
      ...
      {filteredTasks.length === 0 && (
        <div className="text-center py-12">
          ...
          <h3 className="text-xl font-semibold text-pr-text-1 mb-2">No tasks found</h3>
          <p className="text-pr-text-2">Try adjusting your search criteria or create a new task.</p>
        </div>
      )}
...
function TaskCard({ task }: { task: TaskType }) {
  ...
  return (
    <div className="bg-pr-surface-card rounded-xl p-6 shadow-sm border border-pr-surface-3 hover:shadow-md transition-shadow">
      <div className="flex items-start justify-between mb-4">
        ...
        <div className="text-right">
          ...
          <p className="text-sm text-pr-text-2">{task.currency_type}</p>
        </div>
      </div>

      <h3 className="text-lg font-semibold text-pr-text-1 mb-2">{task.title}</h3>
      <p className="text-pr-text-2 text-sm mb-4 line-clamp-3">{task.description}</p>
      ...
      <div className="flex items-center justify-between">
        <span className="text-sm text-pr-text-2">{task.category}</span>
        <button
          onClick={handleApply}
...
function CreateTaskModal({ onClose, onSuccess }: { onClose: () => void; onSuccess: () => void }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
      <div className="bg-pr-surface-card rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div className="p-6 border-b border-pr-surface-3">
          <h2 className="text-2xl font-bold text-pr-text-1">Create New Task</h2>
        </div>

        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          <div>
            <label className="block text-sm font-medium text-pr-text-1 mb-2">Title</label>
            <input
              type="text"
              required
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <label className="block text-sm font-medium text-pr-text-1 mb-2">Category</label>
              <select
            ...
            <div>
              <label className="block text-sm font-medium text-pr-text-1 mb-2">Difficulty</label>
              <select
            ...
            <div>
              <label className="block text-sm font-medium text-pr-text-1 mb-2">Reward Amount ($)</label>
              <input
                type="number"
                min="1"
                required
          ...
          <div className="flex space-x-4">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 px-4 py-2 border border-pr-surface-3 text-pr-text-1 rounded-lg hover:bg-pr-surface-2 transition-colors"
              disabled={loading}
            >
              Cancel
            </button>
