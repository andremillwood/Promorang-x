  ...
  useEffect(() => {
    const checkSession = async () => {
      try {
        ...
        // Fetch user profile with valid token from /auth/me
        // /auth/me is more robust for demo accounts than /users/me
        try {
          console.log('[Auth] Restoring session from /auth/me...');
          const response = await api.get('/auth/me');
          console.log('[Auth] Profile restoration response:', response);

          // The response from /auth/me is { success: true, user: {...} }
          const userData = (response as any)?.user;

          if (userData && userData.id) {
            console.log('[Auth] ✅ Session restored successfully:', userData.email);
            setUser(userData);
            return;
          }

          console.warn('[Auth] ⚠️ No valid user data in profile response:', response);
        } catch (error) {
          console.error('[Auth] ❌ Failed to fetch user profile during restoration:', error);
          // If we got a 401, the token is invalid; clear it
