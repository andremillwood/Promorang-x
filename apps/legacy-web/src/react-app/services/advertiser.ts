...
type ActivationMetric = Record<string, unknown>;

export type ActivationTargetAudience = Record<string, unknown>;
...
export interface ActivationSummary {
  ...
  description?: string;
  target_audience?: ActivationTargetAudience;
  created_at: string;
  updated_at: string;
  campaign_maturity?: string;
  maturity_progress?: number;
  metrics?: ActivationMetric[];
  latest_metrics?: ActivationMetric | null;
  performance?: {
    impressions: number;
    clicks: number;
    conversions: number;
  ...
  coupons?: ActivationMetric[];
  events?: ActivationMetric[];
  content_items?: ActivationMetric[];
  drops?: ActivationMetric[];
  stats?: {
    total_drops: number;
...
export interface ActivationDetailPayload {
  campaign: ActivationSummary;
  metrics: ActivationMetric[];
  content: ActivationMetric[];
  coupons: ActivationMetric[];
}
...
export const advertiserService = {
  ...
  async listCampaigns(): Promise<ActivationSummary[]> {
    try {
      const response = await apiFetch('/api/advertisers/campaigns');
      const result = await parseJson<ApiResponse<{ campaigns: ActivationSummary[] }>>(response);

      if (!response.ok) {
        const message = result?.message || `Failed to load activations (${response.status})`;
        throw new Error(message);
      }

      if (!result || result.status !== 'success') {
        throw new Error(result?.message || 'Failed to load activations');
      }

      return Array.isArray(result.data.campaigns) ? result.data.campaigns : [];
    } catch (error) {
      console.error('[Advertiser API] listActivations', error);
      return [];
    }
  },

  async getCampaign(campaignId: string): Promise<ActivationDetailPayload | null> {
    try {
      const response = await apiFetch(`/api/advertisers/campaigns/${campaignId}`);
      const result = await parseJson<ApiResponse<ActivationDetailPayload>>(response);

      if (!response.ok) {
        const message = result?.message || `Failed to load activation (${response.status})`;
        throw new Error(message);
      }

      if (!result || result.status !== 'success') {
        throw new Error(result?.message || 'Failed to load activation');
      }

      return result.data;
    } catch (error) {
      console.error('[Advertiser API] getActivation', error);
      return null;
    }
  },
  ...
  async updateCampaign(campaignId: string, updates: Partial<ActivationSummary>): Promise<ActivationSummary | null> {
    try {
      const response = await apiFetch(`/api/advertisers/campaigns/${campaignId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
      ...
      const result = await parseJson<ApiResponse<{ campaign: ActivationSummary }>>(response);

      if (!response.ok) {
        const message = result?.message || `Failed to update activation (${response.status})`;
        throw new Error(message);
      }

      if (!result || result.status !== 'success') {
        throw new Error(result?.message || 'Failed to update activation');
      }

      return result.data.campaign;
    } catch (error) {
      console.error('[Advertiser API] updateActivation', error);
      throw error;
    }
  },
  ...
  async deleteCampaign(campaignId: string): Promise<boolean> {
    try {
      ...
      if (!response.ok) {
        const message = result?.message || `Failed to delete activation (${response.status})`;
        throw new Error(message);
      }

      return true;
    } catch (error) {
      console.error('[Advertiser API] deleteActivation', error);
      throw error;
    }
  },
  ...
  async addCampaignFunds(
    ...
    try {
      ...
      console.error('[Advertiser API] addActivationFunds', error);
      throw error;
    }
  },
  ...
  async addCampaignContent(
    ...
    try {
      ...
      console.error('[Advertiser API] addActivationContent', error);
      throw error;
    }
  },
  ...
  async getCampaignDrops(campaignId: string): Promise<DropType[]> {
    try {
      ...
      console.error('[Advertiser API] getActivationDrops', error);
      return [];
    }
  },
