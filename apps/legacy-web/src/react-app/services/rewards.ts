...
class RewardsService {
  ...
  async getCoupon(assignmentId: string): Promise<UserCouponDetail> {
    ...
    return (await response.json()) as UserCouponDetail;
  }
  ...
  async getStats(): Promise<RewardStats> {
    ...
    return (await response.json()) as RewardStats;
  }
  ...
  formatCouponValue(coupon: UserCoupon): string {
    ...
    if (coupon.value_unit === 'verified_credits') {
      return `${coupon.value} verified_credits`;
    }
