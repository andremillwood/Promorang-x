...
export interface CheckoutReactivation {
...
export const paymentsService = {
  ...
  async startCheckout(reactivation: CheckoutReactivation): Promise<CheckoutResponse> {
    const provider = reactivation.provider || normaliseProvider(PAYMENT_CONFIG.defaultProvider);

    if (!provider || !reactivation.planId) {
    ...
    try {
      const response = await apiFetch('/api/payments/checkout', {
        ...
        body: JSON.stringify({
          provider,
          plan_id: reactivation.planId,
          success_url: reactivation.successUrl,
          cancel_url: reactivation.cancelUrl,
          metadata: reactivation.metadata,
        }),
      });
