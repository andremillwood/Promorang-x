...
interface PayoutReactivation {
  id: string;
  period: string;
  reactivationed_amount_cents: number;
...
export default function MatrixDashboard() {
  ...
  const [payouts, setPayouts] = useState<PayoutReactivation[]>([]);
  ...
  const fetchPayouts = useCallback(async () => {
    try {
      const data = await apiFetch<{ payouts: PayoutReactivation[] }>('/matrix/payouts');
  ...
  const handleReactivationPayout = async () => {
    try {
      await apiFetch('/matrix/payouts/reactivation', {
        method: 'POST',
        body: JSON.stringify({})
      });
      fetchPayouts();
      ...
      console.error('Failed to reactivation payout:', err);
    }
  };
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gradient-to-r from-green-500 to-emerald-500 rounded-xl p-6 text-white">
              <div className="flex items-center justify-between">
                ...
                <Button
                  onClick={handleReactivationPayout}
                  ...
                  <div key={payout.id} className="p-4 flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-pr-text-2">
                        {payout.paid_at ? `Paid ${formatDate(payout.paid_at)}` : `Reactivationed ${formatDate(payout.created_at)}`}
                      </p>
                    </div>
                    <div className="text-right">
                      <p className="font-bold text-pr-text-1">{formatCents(payout.approved_amount_cents || payout.reactivationed_amount_cents)}</p>
