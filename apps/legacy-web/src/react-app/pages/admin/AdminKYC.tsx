...
interface VerificationReactivation {
    id: string;
    user_id: string;
...
export default function AdminKYC() {
    const [reactivations, setReactivations] = useState<VerificationReactivation[]>([]);
    const [loading, setLoading] = useState(true);
    const [selectedReactivation, setSelectedReactivation] = useState<VerificationReactivation | null>(null);
    ...
    const handleAction = async (action: 'approve' | 'reject') => {
        ...
        try {
            const res = await fetch('/api/admin/kyc/action', {
                ...
                body: JSON.stringify({
                    verificationId: selectedReactivation.id,
                    action,
                    reason: actionReason
                })
            });
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-gray-400 mt-2">Manage identity verification reactivations.</p>
                </div>
            ...
            {reactivations.length === 0 ? (
                <div className="bg-gray-800 rounded-xl p-12 text-center border border-gray-700">
                    ...
                    <p className="text-gray-400 mt-2">There are no pending verification reactivations at this time.</p>
                </div>
            ) : (
                    ...
                    <div className="space-y-6">
                        <div className="flex items-center justify-between border-b border-gray-200 pb-4">
                            <div>
                                ...
                                <p className="text-sm text-gray-500">Applicant: {selectedReactivation.user.display_name}</p>
                            </div>
                        </div>

                        <div className="bg-gray-100 rounded-lg p-4 flex items-center justify-center min-h-[300px]">
                            <img
                                src={selectedReactivation.document_url}
                                alt="Document ID"
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-1">
                                ...
                                <p className="font-medium text-gray-900 capitalize">{selectedReactivation.document_type.replace('_', ' ')}</p>
                            </div>
                            <div className="space-y-1">
                                <label className="text-xs font-semibold text-gray-500 uppercase">User Email</label>
                                <p className="font-medium text-gray-900">{selectedReactivation.user.email}</p>
                            </div>
                        </div>
