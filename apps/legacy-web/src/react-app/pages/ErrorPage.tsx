...
export default function ErrorPage({ title: propTitle, message: propMessage }: ErrorPageProps = {}) {
  ...
  return (
    <div className="min-h-screen-dynamic bg-gradient-to-br from-gray-50 to-gray-100 flex items-center justify-center px-4">
      <div className="max-w-lg w-full bg-pr-surface-card rounded-2xl shadow-xl overflow-hidden">
        {/* Header */}
        <div className="bg-gradient-to-r from-red-500 to-red-600 px-8 py-6 text-white">
          <div className="flex items-center space-x-3">
            <div className="w-12 h-12 bg-pr-surface-card/20 rounded-full flex items-center justify-center">
        ...
        <div className="px-8 py-6">
          ...
          <div className="bg-pr-surface-2 rounded-lg p-4 mb-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-pr-text-1">Error Reference</p>
                <p className="text-lg font-mono text-pr-text-1">{errorCode}</p>
              </div>
              <button
                onClick={handleCopyErrorCode}
                className="flex items-center space-x-1 px-3 py-2 bg-pr-surface-card rounded-lg hover:bg-pr-surface-2 transition-colors"
              >
                {copied ? (
                  <CheckCircle className="w-4 h-4 text-green-600" />
                ) : (
                  <Copy className="w-4 h-4 text-pr-text-2" />
                )}
          ...
          <div className="space-y-3">
            ...
            <button
              ...
              className="w-full bg-pr-surface-2 hover:bg-pr-surface-3 text-pr-text-1 px-6 py-3 rounded-lg font-medium transition-colors flex items-center justify-center space-x-2"
            >
