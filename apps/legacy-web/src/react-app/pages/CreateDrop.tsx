...
const DROP_TYPES = [
  {
    ...
    description: 'Reactivation UGC, reviews, or original content from creators',
    icon: Camera,
    color: '#3B82F6',
    requiresProof: true,
    ...
    switch (currentStep) {
      ...
      case 3:
        if (selectedDropType?.gemPoolRequired && formData.gem_pool_total < 100) {
          setError('Gem pool must be at least 100 verified_credits');
          return false;
        }
        return true;
      default:
      ...
      <div className="space-y-6 max-w-2xl">
        ...
        <div className="p-6 rounded-2xl border border-pr-surface-3 bg-pr-surface-1">
          ...
          <p className="text-sm text-pr-text-2 mb-4">
            Total verified_credits to distribute to participants. This amount will be held in escrow.
          </p>
          ...
          <div className="flex items-center gap-4 mb-4">
            <input
              ...
              min={100}
            />
            <span className="text-lg font-semibold text-pr-text-2">💎 Verified Credits</span>
          </div>
