...
import VerifiedCreditSupportModal from '@/react-app/components/VerifiedCreditSupportModal';
        ...
        notification.innerHTML = `
          <div class="flex items-center space-x-2">
            ...
            <div>
              ...
              <div class="text-xs opacity-90">Tipped ${amount} verified_credits to ${content.creator_name || content.creator_username || 'creator'}</div>
            </div>
          </div>
        `;
        document.body.appendChild(notification);
  ...
  const handleSponsorContent = async (gemAmount: number, boostMultiplier: number, durationHours: number) => {
    ...
    try {
      const response = await fetch(withApiBase(`/api/content/${content.id}/sponsor`), {
        ...
        credentials: 'include',
        body: JSON.stringify({
          verified_credits_allocated: gemAmount,
                ...
                <div className="bg-gradient-to-r from-purple-50 to-pink-50 border border-purple-200 rounded-xl p-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div className="bg-white/50 p-4 rounded-lg">
                      ...
                      <span className="text-2xl font-bold text-purple-900">{sponsorshipData.total_verified_credits_allocated || sponsorshipData.verified_credits_allocated} CREDITS</span>
                    </div>
                    <div className="bg-white/50 p-4 rounded-lg">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-pr-surface-card rounded-xl border border-pr-surface-3 p-6 sticky top-24">
            ...
            <div className="space-y-3">
              <button
                disabled
                className="w-full bg-gray-200 text-gray-500 px-6 py-4 rounded-xl font-bold text-lg cursor-not-allowed flex items-center justify-center gap-2"
              >
              ...
              <button
                disabled
                className="w-full bg-gray-100 text-gray-400 border border-gray-200 px-6 py-3 rounded-lg font-medium cursor-not-allowed flex items-center justify-center space-x-2"
              >
                <Target className="w-4 h-4" />
                <span>Forecasts Paused</span>
              </button>
        ...
        content && (
          <>
            ...
            <SponsorshipModal
              ...
              contentTitle={content.title}
              userVerifiedCredits={userData?.verified_credits_balance || 0}
            />

            {/* Support Modal */}
            <VerifiedCreditSupportModal
              isOpen={tipModalOpen}
              onClose={() => setTipModalOpen(false)}
              recipientName={content.creator_name || content.creator_username || 'Creator'}
              recipientId={content.creator_id.toString()}
