...
export default function EditActivation() {
  ...
  // Budget allocation
  const [totalBudgetCredits, setTotalBudgetCredits] = useState(0);
  const [promoShareContribution, setPromoShareContribution] = useState(0);
    ...
    const fetchActivationData = async () => {
      ...
      try {
        ...
        if (data && data.campaign) {
          ...
          setActivationName(c.name || '');
          setActivationDescription(c.description || '');
          setStartDate(new Date(c.start_date));
          setEndDate(c.end_date ? new Date(c.end_date) : null);
          setStatus(c.status || 'active');
          setTotalBudgetCredits(c.total_gem_budget || 0);
          setPromoShareContribution(c.promoshare_contribution || 0);

          // Map Content Items
          if (c.content_items) {
            setContentItems((c.content_items as any[]).map(item => ({
              id: item.id,
              type: item.content_type || 'link',
              title: item.title || '',
              url: item.url || '',
          ...
          // Map Side Activations
          if (c.drops) {
            setDrops((c.drops as any[]).map(drop => ({
              id: drop.id,
              title: drop.title || '',
              description: drop.description || '',
              type: drop.drop_type || 'share',
              gemReward: drop.gem_reward || 10,
              keysCost: drop.keys_cost || 1,
              maxParticipants: drop.max_participants || 100,
          ...
          // Map Events
          if (c.events) {
            setEvents((c.events as any[]).map(event => ({
              id: event.id,
              title: event.title || '',
        ...
        console.error('Failed to fetch activation for editing:', err);
        toast({
          title: 'Error',
  ...
  // Management functions (same as NewCampaign.tsx)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!activationName.trim()) {
      toast({ title: 'Validation Error', description: 'Please enter an activation name', type: 'warning' });
      return;
    }

    if (drops.length === 0) {
      toast({ title: 'Validation Error', description: 'Please add at least one task', type: 'warning' });
      return;
    }
    ...
    try {
      await advertiserService.updateCampaign(campaignId, {
        name: activationName,
        description: activationDescription,
        status: status as any,
        start_date: startDate.toISOString(),
        end_date: endDate?.toISOString() || null,
        content_items: contentItems as any,
        drops: drops as any,
        events: events as any,
        coupons: coupons as any,
        total_gem_budget: totalBudgetNeeded,
      ...
      navigate(`/advertiser/activations/${campaignId}`);
    } catch (error: any) {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-64 space-y-4">
        <Loader2 className="h-8 w-8 animate-spin text-pr-text-2" />
        <p className="text-pr-text-2">Loading activationline data...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto pb-20">
      ...
      <div className="flex items-center space-x-4">
        ...
        <div>
          <h2 className="text-2xl font-bold text-pr-text-1">Edit Activation</h2>
          <p className="text-sm text-pr-text-2">
            Update your bundled content, tasks, and events
          </p>
        </div>
      </div>
        ...
        {steps.map((step, index) => (
          <div key={step.number} className="flex items-center">
            ...
            {index < steps.length - 1 && (
              <div className="w-8 h-0.5 mx-2 bg-pr-surface-3" />
            )}
          </div>
        ))}
      </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <Textarea
                  ...
                  placeholder="What is this activation about?"
                  rows={3}
                />
              </div>
              ...
              <div>
                <label className="block text-sm font-medium text-pr-text-1 mb-2">Activationline Status</label>
              ...
              <div className="space-y-4">
                {coupons.map((coupon, index) => (
                  <div key={coupon.id} className="p-4 bg-pr-surface-2 rounded-lg space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="block text-xs text-pr-text-2 mb-1">Discount Value</label>
                        <Input
                          type="number"
                      ...
                      <div>
                        <label className="block text-xs text-pr-text-2 mb-1">Quantity</label>
                        <Input
                          type="number"
        ...
        {currentStep === 6 && (
          <div className="space-y-6">
            <div className="bg-pr-surface-card rounded-xl p-6 border border-pr-surface-3">
              <h3 className="text-lg font-semibold text-pr-text-1 mb-4">Update Summary</h3>
              <div className="space-y-4">
                <div className="flex justify-between py-2 border-b border-pr-surface-3">
                  <span className="text-pr-text-2">Activation Name</span>
                  <span className="font-medium text-pr-text-1">{activationName}</span>
                </div>
                <div className="flex justify-between py-2 border-b border-pr-surface-3">
                  <span className="text-pr-text-2">Tasks</span>
                  <span className="font-medium text-pr-text-1">{drops.length} tasks</span>
                </div>
                <div className="flex justify-between py-2 border-b border-pr-surface-3">
                  <span className="text-pr-text-2">Events Bundled</span>
                  <span className="font-medium text-pr-text-1">{events.length} events</span>
                </div>
                <div className="flex justify-between py-2 border-b border-pr-surface-3 font-bold text-pr-text-1">
                  <span>New Credit Budget</span>
                  <span>{totalBudgetNeeded} CRD</span>
                </div>
              </div>
            </div>
            ...
            <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
              <p className="text-sm text-yellow-700">
                <strong>Note:</strong> Updating an activation will synchronize all tasks and events. Removed items will be permanently deleted from this activation.
              </p>
            </div>
          </div>
        )}
