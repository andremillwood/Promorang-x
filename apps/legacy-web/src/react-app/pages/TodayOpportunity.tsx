...
import DropSubmissionModal from '@/react-app/components/DropSubmissionModal';
...
export default function TodayOpportunity() {
    ...
    const demoFallback = {
        title: "Share Your Presence Moment ☕",
        subtitle: "Confirm your presence at a local coffee spot to index your participation in the network.",
        description: "Visit any local coffee shop, confirm your presence, and optionally record a reflection. We'll index your Impact within 24 hours.",
        reward_amount: 50,
        category: "Moment",
    };

    const displayTitle = payload?.title && payload.title !== "Today's Featured Opportunity"
        ? payload.title
        : demoFallback.title;
    const displaySubtitle = payload?.subtitle && payload.subtitle !== "Explore today's featured drop"
        ? payload.subtitle
        : demoFallback.subtitle;
    const displayDescription = drop?.description || demoFallback.description;
    const displayReward = payload?.reward_amount || demoFallback.reward_amount;
    const displayCategory = drop?.category || demoFallback.category;
    const hasRealDrop = !!(drop?.id);
    ...
    const handleAction = () => {
        if ((drop as any)?.drop_role === 'BUY' || type === 'buy') {
            setIsProofModalOpen(true);
        } else if (type === 'reward' && drop?.id) {
    ...
    return (
        <div className="min-h-screen-dynamic bg-pr-surface-1 pt-16">
            {/* Header / Back */}
            <div className="max-w-2xl mx-auto px-4 py-4 flex items-center justify-between">
                ...
                <div className="text-right">
                    <span className="text-[10px] font-bold text-pr-text-3 uppercase tracking-widest">Moment Anchor</span>
            ...
            <div className="max-w-2xl mx-auto px-4 pb-20">
                ...
                <div className={`relative overflow-hidden rounded-3xl border ${currentTheme.border} bg-gradient-to-br ${currentTheme.bg} p-8 mb-8`}>
                    <div className="relative z-10">
                        <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full ${currentTheme.badge} text-xs font-bold uppercase tracking-widest mb-6`}>
                            {currentTheme.icon}
                            {type === 'buy' || drop?.drop_role === 'BUY' ? "Purchase Required" :
                                type === 'reward' ? "Featured Activation" :
                                    type === 'multiplier' ? "Multiplier Event" : "Lucky Draw Event"}
                        </div>
                        ...
                        <div className="flex flex-wrap gap-4">
                            {type === 'reward' && (
                                <div className="px-4 py-2 bg-black/40 backdrop-blur-md border border-white/10 rounded-2xl flex items-center gap-3">
                                    ...
                                    <div>
                                        <p className="text-[10px] text-pr-text-3 font-bold uppercase">Index Target</p>
                                        <p className="text-sm font-black text-pr-text-1">{displayReward} Impact</p>
                                    </div>
                                </div>
                            )}
                            {type === 'multiplier' && (
                ...
                {/* Details Section - Always show for reward type */}
                {type === 'reward' && (
                    <div className="bg-pr-surface-2 rounded-2xl border border-pr-border/50 p-6 mb-8">
                        <h2 className="text-xs font-bold text-pr-text-3 uppercase tracking-widest mb-4 flex items-center gap-2">
                            ...
                            {!hasRealDrop && <span className="text-[10px] text-orange-400 ml-2">(Demo)</span>}
                        </h2>
                        <div className="space-y-4">
                            <div>
                                <h3 className="text-lg font-bold text-pr-text-1 mb-1">{drop?.title || displayTitle}</h3>
                ...
                <div className="grid grid-cols-1 gap-6 mb-12">
                    <div className="flex items-start gap-4">
                        ...
                        <div>
                            <h3 className="font-bold text-pr-text-1">Accept the Invitation</h3>
                            <p className="text-xs text-pr-text-2">Commit to today's invitation by clicking the button below.</p>
                        </div>
                    </div>
                    <div className="flex items-start gap-4">
                        <div className="p-3 bg-pr-surface-3 rounded-xl border border-pr-border/50 text-pr-text-1 font-black shadow-inner">
                        ...
                        <div>
                            <h3 className="font-bold text-pr-text-1">Verify Presence</h3>
                            <p className="text-xs text-pr-text-2">Reach the location and confirm your presence to index your participation.</p>
                        </div>
                    </div>
                    <div className="flex items-start gap-4">
                        <div className="p-3 bg-pr-surface-3 rounded-xl border border-pr-border/50 text-pr-text-1 font-black shadow-inner">
                        ...
                        <div>
                            <h3 className="font-bold text-pr-text-1">Confirm Standing</h3>
                            <p className="text-xs text-pr-text-2">Your verified presence increases your Impact and strengthens your personal story.</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="fixed bottom-0 left-0 right-0 p-4 bg-pr-surface-1/80 backdrop-blur-xl border-t border-pr-border/50 z-50">
                    <div className="max-w-2xl mx-auto">
                        ...
                        >
                            {(drop?.drop_role === 'BUY' || type === 'buy') ? "Submit Receipt" : (type === 'reward' ? "Accept Invitation" : "View Invitations")}
                ...
                {drop && (
                    <DropSubmissionModal
                        ...
                        onSuccess={() => {
                            ...
                            // Maybe refresh state or show success message
                            navigate('/today');
                        }}
                        initialType="receipt"
                    />
