...
export default function Create() {
  ...
  const [fundWithVerifiedCredits, setFundWithVerifiedCredits] = useState(false);
  const [creditFundingAmount, setCreditFundingAmount] = useState(0);
  ...
  const handleCreatePost = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Calculate share price based on credit funding
      const finalPostData = {
        ...postData,
        share_price: fundWithVerifiedCredits && creditFundingAmount > 0 ? creditFundingAmount / postData.total_shares : 0.0
      };

      console.log('Sending content creation activation:', finalPostData);
      ...
      if (response.ok) {
        ...
        // If funding with verified credits, make a funding activation
        if (fundWithVerifiedCredits && creditFundingAmount > 0) {
          try {
            await fetch(`/api/content/${result.contentId}/fund`, {
              ...
              credentials: 'include',
              body: JSON.stringify({
                amount: creditFundingAmount,
                currency_type: 'Verified Credits'
              })
            });
          } catch (fundingError) {
        ...
        setFundWithVerifiedCredits(false);
        setCreditFundingAmount(0);
        removeUploadedFile();
        setUploadMode('url');
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-pr-text-1">Create & Submit</h1>
          <p className="text-pr-text-2 mt-2">Document your participation, host your own moments, and unlock verified rewards.</p>
        </div>
        ...
        <div className="text-right">
          <div className="flex items-center gap-2 justify-end mb-1">
            ...
            <span className="text-sm font-bold text-pr-text-1">Build Your Wallet</span>
          </div>
          <p className="text-xs text-gray-400">Every verified submission builds your history.</p>
        </div>
      </div>
        ...
        <div className="border-b border-pr-surface-3">
          <nav className="flex space-x-8 px-6">
            <button
              ...
              className={`py-4 border-b-2 font-medium text-sm ${activeTab === 'post'
            ...
            <button
              ...
              className={`py-4 border-b-2 font-medium text-sm ${activeTab === 'upload'
                ? 'border-orange-500 text-orange-600'
                : 'border-transparent text-pr-text-2 hover:text-pr-text-1'
                }`}
            >
        ...
        <div className="p-6">
          {activeTab === 'post' && (
            <form onSubmit={handleCreatePost} className="space-y-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                  {PLATFORMS.map((platform) => (
                    <button
                      ...
                      className={`p-4 border rounded-lg flex flex-col items-center space-y-2 transition-all ${postData.platform === platform
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-medium text-pr-text-1 mb-2">Content Title</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="block text-sm font-medium text-pr-text-1 mb-2">Platform URL</label>
                  <input
                    type="url"
                    required
              ...
              <div>
                <label className="block text-sm font-medium text-pr-text-1 mb-3">Media Content (Optional)</label>
                ...
                <div className="flex space-x-4 mb-4">
                  <button
                    ...
                    className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${uploadMode === 'url'
                      ? 'bg-orange-100 text-orange-700 border border-orange-300'
                      : 'bg-pr-surface-2 text-pr-text-2 border border-pr-surface-3 hover:bg-pr-surface-3'
                      }`}
                  >
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${uploadMode === 'file'
                      ? 'bg-orange-100 text-orange-700 border border-orange-300'
                      : 'bg-pr-surface-2 text-pr-text-2 border border-pr-surface-3 hover:bg-pr-surface-3'
                      }`}
                  >
                  ...
                  <div className="space-y-4">
                    {!uploadedFile ? (
                      <div
                        ...
                        className="border-2 border-dashed border-pr-surface-3 rounded-lg p-8 text-center cursor-pointer hover:border-orange-400 hover:bg-orange-50 transition-all"
                      >
                        <Upload className="w-8 h-8 text-gray-400 mx-auto mb-3" />
                        <p className="text-pr-text-2 mb-2">Click to upload or drag and drop</p>
                      ...
                      <div className="border border-pr-surface-3 rounded-lg p-4">
                        <div className="flex items-center justify-between mb-3">
                          <div className="flex items-center space-x-3">
                            ...
                            <div>
                              <p className="font-medium text-pr-text-1">{uploadedFile.name}</p>
                              <p className="text-sm text-pr-text-2">
              ...
              <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
                ...
                {/* Fund with Verified Credits Toggle */}
                <div className="mb-4">
                  <label className="flex items-center space-x-3 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={fundWithVerifiedCredits}
                      onChange={(e) => {
                        setFundWithVerifiedCredits(e.target.checked);
                        if (!e.target.checked) {
                          setCreditFundingAmount(0);
                    ...
                    />
                    <div>
                      <span className="text-sm font-medium text-purple-700">Fund content with verified credits</span>
                      <p className="text-xs text-purple-600">Give your content initial value by funding it with verified credits</p>
                    </div>
                  </label>
                </div>

                {/* Credit Funding Amount (only shown if funding enabled) */}
                {fundWithVerifiedCredits && (
                  <div className="mb-4">
                    <label className="block text-sm font-medium text-purple-700 mb-2">Credit Funding Amount</label>
                    <input
                      type="number"
                      min="1"
                      step="1"
                      value={creditFundingAmount || ''}
                      onChange={(e) => {
                        const amount = Number(e.target.value);
                        setCreditFundingAmount(amount);
                        setPostData({
                          ...postData,
                      ...
                      className="w-full px-4 py-2 border border-purple-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                      placeholder="Enter credits to fund content"
                    />
                    <div className="text-xs text-purple-600 mt-1 space-y-1">
                      <p>This will be split across all {postData.total_shares} shares</p>
                      {creditFundingAmount > 0 && (
                        <p>Share price: {(creditFundingAmount / postData.total_shares).toFixed(4)} credits per share</p>
                      )}
                    </div>
                  </div>
                )}
                ...
                <div className="bg-purple-100 border border-purple-300 rounded-lg p-3">
                  <p className="text-sm text-purple-700">
                    ...
                    {fundWithVerifiedCredits && creditFundingAmount > 0 && (
                      <>
                        <br />
                        <strong>Total funding:</strong> {creditFundingAmount} verified credits
                        <br />
                        <strong>Value per share:</strong> {(creditFundingAmount / postData.total_shares).toFixed(4)} verified credits
                      </>
                    )}
                    {!fundWithVerifiedCredits && (
                      <>
          ...
          {activeTab === 'upload' && (
            <div className="space-y-6">
              {/* Upload Area */}
              <div className="border-2 border-dashed border-pr-surface-3 rounded-lg p-12 text-center">
                <Upload className="w-12 h-12 text-gray-400 mx-auto mb-4" />
                <h3 className="text-lg font-semibold text-pr-text-1 mb-2">Upload your content</h3>
              ...
              {/* Coming Soon */}
              <div className="bg-pr-surface-2 border border-pr-surface-3 rounded-lg p-8 text-center">
                <h3 className="text-lg font-semibold text-pr-text-1 mb-2">Direct Upload Coming Soon</h3>
