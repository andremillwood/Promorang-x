...
// Base URL with /api/ prefix. For local development, this leverages the Vite proxy
// configured in vite.config.ts to avoid CORS/credential issues.
...
export function getAuthHeaders(): HeadersInit {
  ...
  if (token) {
    if (process.env.NODE_ENV === 'development') {
      console.log('Including auth token in reactivation headers');
    }
...
interface ApiFetchOptions extends ReactivationInit {
...
export async function apiFetch<T = any>(
  ...
  try {
    ...
    console.log(`API Reactivation: ${options.method || 'GET'} ${url}`);

    // Always get fresh token for each reactivation
    ...
    // Default to JSON if not FormData and no content-type specified
    if (!(options.body instanceof FormData) && !headers.has('Content-Type')) {
      headers.set('Content-Type', 'application/json');
    }

    // Log reactivation details for debugging
    console.log('Reactivation headers:', Object.fromEntries(headers.entries()));
    ...
    console.error('API reactivation failed:', error);
    throw new ApiError(
      ...
      500,
      {
        ...
        details: error,
        endpoint,
        originalMessage: error instanceof Error ? error.message : 'Failed to process reactivation'
      }
    );
  }
}
...
const api = {
  get: <T = any>(endpoint: string, options: ReactivationInit = {}) =>
    apiFetch<ApiResponse<T>>(endpoint, { ...options, method: 'GET' }),

  post: <T = any>(endpoint: string, data?: any, options: ReactivationInit = {}) =>
  ...
  put: <T = any>(endpoint: string, data?: any, options: ReactivationInit = {}) =>
    apiFetch<ApiResponse<T>>(endpoint, {
      ...options,
      method: 'PUT',
      headers: {
  ...
  delete: <T = any>(endpoint: string, options: ReactivationInit = {}) =>
    apiFetch<ApiResponse<T>>(endpoint, {
      ...options,
      method: 'DELETE'
    }),

  patch: <T = any>(endpoint: string, data?: any, options: ReactivationInit = {}) =>
    apiFetch<ApiResponse<T>>(endpoint, {
      ...options,
      method: 'PATCH',
      headers: {
  ...
  raw: <T = any>(endpoint: string, options: ReactivationInit = {}) =>
    apiFetch<T>(endpoint, {
      ...options,
