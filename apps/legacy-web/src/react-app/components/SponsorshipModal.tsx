...
export default function SponsorshipModal({
  ...
  const handleSponsor = () => {
    ...
    onSponsor(selectedPackage.Credits, selectedPackage.multiplier, selectedPackage.duration);
  };

  const balancePercent = Math.min(100, (userVerifiedCredits / 500) * 100);
      ...
      <div className="flex max-h-[85vh] flex-col overflow-hidden rounded-2xl bg-pr-surface-card">
        ...
        <div className="flex-1 overflow-y-auto px-6 pb-6 space-y-6">
          ...
          <div>
            <div className="mb-3 flex items-center justify-between">
              <span className="text-sm font-medium text-pr-text-1">Your Verified Credits</span>
              <span className="text-lg font-semibold text-blue-600">{userVerifiedCredits} Credits</span>
            </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid gap-4">
              {sponsorshipPackages.map((pkg) => (
                <div
                  key={pkg.id}
                  className={`cursor-pointer rounded-xl border p-4 transition-all duration-200 ${selectedPackage?.id === pkg.id
                      ? 'border-purple-500 bg-purple-50 shadow-lg scale-[1.02]'
                      : 'border-pr-surface-3 hover:border-pr-surface-3 hover:shadow-md'
                    } ${pkg.Credits > userVerifiedCredits ? 'cursor-not-allowed opacity-50' : ''}`}
              onClick={() => {
                if (pkg.Credits <= userVerifiedCredits) {
              ...
              <div className="flex items-start justify-between">
                ...
                <div className="text-right">
                  <p className="text-2xl font-bold text-purple-600">{pkg.Credits}</p>
                  <p className="text-xs text-pr-text-2">Credits</p>
                  {pkg.Credits > userVerifiedCredits && (
                  <p className="mt-1 text-xs text-red-500">Insufficient Credits</p>
                      )}
                </div>
              </div>
            </div>
          ...
          <div className="rounded-xl border border-purple-200 bg-gradient-to-r from-purple-50 to-pink-50 p-4">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <span className="ml-2 font-medium text-purple-900">{selectedPackage.Credits} Credits</span>
              </div>
            </div>
        ...
        <div className="space-y-3">
          <button
            ...
            disabled={!selectedPackage || selectedPackage?.Credits > userVerifiedCredits}
          className="w-full rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 px-6 py-4 font-semibold text-white shadow-lg transition-all duration-200 hover:from-purple-700 hover:to-indigo-700 disabled:cursor-not-allowed disabled:opacity-50"
            >
          {selectedPackage ? `Sponsor for ${selectedPackage.Credits} Credits` : 'Select a sponsorship package'}
        </button>
        <button
          onClick={onClose}
