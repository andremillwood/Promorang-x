...
import api, { getActiveMerchantAccountId, setActiveMerchantAccountId } from '../lib/api';
...
export default function MerchantAccountSwitcher() {
    ...
    const [activeAccountId, setActiveAccountId] = useState<string | null>(getActiveMerchantAccountId());
    ...
    useEffect(() => {
        const fetchAccounts = async () => {
            try {
                setLoading(true);
                const data = await api.get<MerchantAccount[]>('/merchant-team/accounts');
                if (data.success && data.accounts) {
                    ...
                    if (!activeAccountId && data.accounts.length > 0) {
                        ...
                        setActiveMerchantAccountId(firstId);
                    }
                }
            } catch (error) {
                console.error('Failed to fetch merchant accounts:', error);
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleSwitch = (accountId: string) => {
        ...
        setActiveMerchantAccountId(accountId);
        setOpen(false);
    ...
    return (
        <div className="relative">
            ...
            >
                <div className="w-6 h-6 rounded bg-emerald-500 overflow-hidden flex items-center justify-center flex-shrink-0">
                ...
                <span className="text-sm font-medium text-pr-text-1 truncate max-w-[120px]">
                    {activeAccount?.name || 'Select Store'}
                </span>
                ...
                <>
                    ...
                    <div className="absolute top-full mt-2 left-0 w-64 bg-pr-surface-1 rounded-xl shadow-xl border border-pr-surface-3 py-2 z-50 overflow-hidden">
                        <div className="px-4 py-2 border-b border-pr-surface-3">
                            <span className="text-xs font-semibold text-pr-text-3 uppercase tracking-wider">Switch Store</span>
                        </div>
                        ...
                        <div className="max-h-60 overflow-y-auto">
                            {accounts.map(account => (
                                <button
                                    ...
                                    className={`w-full flex items-center gap-3 px-4 py-3 hover:bg-pr-surface-2 transition-colors text-left ${activeAccountId === account.id ? 'bg-emerald-500/5' : ''}`}
                                >
                                    <div className={`w-8 h-8 rounded flex items-center justify-center flex-shrink-0 ${activeAccountId === account.id ? 'bg-emerald-500' : 'bg-pr-surface-3'}`}>
                                        {account.logo_url ? (
                                            <img src={account.logo_url} alt={account.name} className="w-full h-full rounded object-cover" />
                                        ) : (
                                            <StoreIcon className={`w-4 h-4 ${activeAccountId === account.id ? 'text-white' : 'text-pr-text-3'}`} />
                                        )}
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <p className={`text-sm font-medium truncate ${activeAccountId === account.id ? 'text-emerald-500' : 'text-pr-text-1'}`}>
                                            {account.name}
                                        </p>
                        ...
                        {loading && (
                            <div className="p-4 text-center">
                                <RefreshCwIcon className="w-5 h-5 animate-spin text-emerald-500 mx-auto" />
                            </div>
                        )}
                    </div>
                </>
