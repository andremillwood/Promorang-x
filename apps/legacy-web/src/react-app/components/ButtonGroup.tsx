...
function PrimaryButton({ variant, text, icon, onClick, disabled }: PrimaryButtonProps) {
  const getVariantStyles = () => {
    switch (variant) {
      ...
      default:
        return 'bg-pr-surface-20 hover:bg-gray-600';
    }
  };
...
function PriceLabel({ content }: { content: ContentPieceType }) {
  ...
  return (
    <div className="text-center">
      <div className="text-lg font-bold text-pr-text-1">
        {sharePrice ? `$${sharePrice.toFixed(2)}` : '—'}
      </div>
      <div className="text-xs text-pr-text-2">per share</div>
    </div>
  );
}
...
function AvailabilityLabel({ content }: { content: ContentPieceType }) {
  ...
  return (
    <div className="text-center">
      ...
      <div className="text-xs text-pr-text-2">available</div>
    </div>
  );
}
...
export default function ButtonGroup({
  ...
  return (
    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 sm:gap-3 mt-4">
      ...
      {shareHint && (
        <p className="text-xs text-pr-text-2 bg-pr-surface-2 border border-gray-100 rounded-lg px-3 py-2">
          {shareHint}
        </p>
      )}
      ...
      <div className="pt-3 border-t border-gray-100">
        <button
          ...
          className="flex items-center justify-center space-x-2 px-4 py-2 text-sm text-pr-text-2 hover:text-purple-600 hover:bg-purple-50 rounded-lg transition-all duration-200 hover:scale-[1.02] active:scale-[0.98] w-full sm:w-auto"
