...
const DEFAULT_UPGRADE_COSTS: Record<string, Record<string, { points: number; verified_credits: number }>> = {
  free: {
    premium: { points: 7500, verified_credits: 200 },
    super: { points: 15000, verified_credits: 450 }
  },
  premium: {
    super: { points: 9000, verified_credits: 275 }
  },
  super: {}
};
...
export default function UserTierUpgradeModal({ user, isOpen, onClose, onSuccess }: UserTierUpgradeModalProps) {
  ...
  const [paymentMethod, setPaymentMethod] = useState<'points' | 'verified_credits'>('points');
  ...
  const normalizeCosts = (rawCosts: Record<string, Record<string, { points: number; verified_credits: number }>>) => {
  ...
  const canAfford = (tier: string, method: 'points' | 'verified_credits') => {
    const cost = availableUpgrades[tier]?.[method];
    if (!cost) return false;

    const balance = method === 'points' ? user.points_balance : user.verified_credits_balance;
    return balance >= cost;
  };
                        ...
                        <div>
                          ...
                          <div className="flex items-center space-x-4 text-sm">
                            ...
                            <div className="flex items-center space-x-1">
                              ...
                              <span>{cost.verified_credits} verified_credits</span>
                            </div>
                          </div>
                        </div>
                      </div>
          ...
          {selectedTier && (
            <div className="mt-6 p-4 bg-pr-surface-2 rounded-lg">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <button
                  onClick={() => setPaymentMethod('verified_credits')}
                  disabled={!canAfford(selectedTier, 'verified_credits')}
                  className={`p-4 rounded-lg border-2 transition-colors ${paymentMethod === 'verified_credits'
                      ? 'border-purple-500 bg-purple-50'
                      : 'border-pr-surface-3 hover:border-pr-surface-3'
                    } ${!canAfford(selectedTier, 'verified_credits') ? 'opacity-50 cursor-not-allowed' : ''}`}
                >
                  <div className="flex items-center justify-center space-x-2 mb-2">
                    <Gem className="w-5 h-5 text-purple-600" />
                    <span className="font-medium">Verified Credits</span>
                  </div>
                  <p className="text-lg font-bold text-purple-600">
                    {availableUpgrades[selectedTier]?.verified_credits}
                  </p>
                  <p className="text-xs text-pr-text-2">
                    Balance: {user.verified_credits_balance}
                  </p>
                  {!canAfford(selectedTier, 'verified_credits') && (
                    <p className="text-xs text-red-600 mt-1">Insufficient balance</p>
                  )}
                </button>
              </div>
