...
export default function ContentFundingModal({
  ...
  const [currency, setCurrency] = useState<'USD' | 'Verified Credits' | 'Points'>('USD');
  ...
  const getCurrencyIcon = (curr: string) => {
    switch (curr) {
      ...
      case 'Verified Credits': return <Gem className="w-4 h-4" />;
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {(['USD', 'Verified Credits', 'Points'] as const).map((curr) => (
                  <button
                    key={curr}
                    type="button"
