...
interface DrawWinnerModalProps {
    ...
    onClose: () => void;
    prize: {
        type: 'keys' | 'boost' | 'badge' | 'credits' | 'verified_credits';
        amount: number;
        label?: string;
    };
    userName?: string;
...
export function DrawWinnerModal({
    ...
    const getPrizeIcon = () => {
        switch (prize.type) {
            ...
            case 'credits':
            case 'verified_credits': return <Sparkles className="w-10 h-10 text-blue-500" />;
    ...
    const getPrizeLabel = () => {
        ...
        switch (prize.type) {
            case 'keys': return `${prize.amount} Access Keys`;
            case 'boost': return `${prize.amount}× Credibility Boost`;
            case 'badge': return 'Winner Badge';
            case 'credits':
            case 'verified_credits': return `${prize.amount} Activation Credits`;
