...
import { CreateDropReactivationSchema, DropTypeSchema, DropDifficultySchema } from '../../shared/types';
...
export default function EditDropModal({
  ...
  const [formData, setFormData] = useState({
    ...
    verified_credits_reward_base: drop.gem_reward_base,
    credits_pool_total: drop.gem_pool_total,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Validate form data
      const validationResult = CreateDropReactivationSchema.safeParse(submitData);
            ...
            <div className="flex items-center space-x-2">
              ...
              {drop.is_paid_drop && (
                <span className="bg-purple-100 text-purple-800 px-3 py-1 rounded-full text-sm font-medium">
                  Paid Drop - Costs keys, rewards verified_credits
                </span>
              )}
          ...
          {drop.is_paid_drop && (
            <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-purple-700 mb-2">Verified Credits Reward</label>
                  <input
                    type="number"
                    min="1"
                    value={formData.verified_credits_reward_base}
                    onChange={(e) => setFormData({ ...formData, verified_credits_reward_base: Number(e.target.value) })}
                    className="w-full px-4 py-2 border border-purple-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                    disabled={loading}
                  />
                </div>
                ...
                <div>
                  <label className="block text-sm font-medium text-purple-700 mb-2">Total Credits Pool</label>
                  <input
                    type="number"
                    min="0"
                    value={formData.credits_pool_total}
                    onChange={(e) => setFormData({ ...formData, credits_pool_total: Number(e.target.value) })}
                    className="w-full px-4 py-2 border border-purple-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                    disabled={loading}
                  />
                </div>
