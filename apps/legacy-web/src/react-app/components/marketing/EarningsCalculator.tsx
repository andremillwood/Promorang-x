...
export default function EarningsCalculator({ onGetStarted }: EarningsCalculatorProps) {
    ...
    // Credibility calculation based on followers
    // Base: ~1 Credibility point per 100 followers for active engagement
    const weeklyCredibility = Math.floor(followers * 0.02); // 2% conversion rate
    const monthlyCredibility = weeklyCredibility * 4;
    const potentialRewards = (monthlyCredibility * 0.01).toFixed(2); // Estimated value in credits
        ...
        <div className="bg-pr-surface-card border border-pr-border rounded-2xl p-8 max-w-2xl mx-auto">
            <div className="text-center mb-8">
                <div className="inline-flex items-center gap-2 bg-pr-surface-2 px-4 py-2 rounded-full mb-4">
                    ...
                    <span className="text-sm font-medium text-pr-text-1">Credibility Calculator</span>
                </div>
                <h3 className="text-2xl md:text-3xl font-bold text-pr-text-1 mb-2">
                    See Your Platform Power
                </h3>
                <p className="text-pr-text-2">
                    Estimate your monthly Credibility score based on your Instagram following
                </p>
            </div>
            ...
            <div className="grid grid-cols-3 gap-4 mb-8">
                <div className="bg-pr-surface-2 rounded-xl p-4 text-center">
                    <div className="text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-blue-500 to-purple-500">
                        {weeklyCredibility.toLocaleString()}
                    </div>
                    <div className="text-xs text-pr-text-2 mt-1">Weekly Credibility</div>
                </div>
                <div className="bg-pr-surface-2 rounded-xl p-4 text-center">
                    <div className="text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-green-500 to-teal-500">
                        {monthlyCredibility.toLocaleString()}
                    </div>
                    <div className="text-xs text-pr-text-2 mt-1">Monthly Credibility</div>
                </div>
                <div className="bg-pr-surface-2 rounded-xl p-4 text-center">
                    <div className="text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-orange-500 to-red-500">
                        ${potentialRewards}
                    </div>
                    <div className="text-xs text-pr-text-2 mt-1">Reward Potential</div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 mb-8">
                <div className="flex items-start gap-3 p-3 bg-pr-surface-2 rounded-lg">
                    ...
                    <div>
                        <div className="text-sm font-bold text-pr-text-1">Brand Activations</div>
                        <div className="text-xs text-pr-text-2">Execute verified tasks for top brands</div>
                    </div>
                </div>
                <div className="flex items-start gap-3 p-3 bg-pr-surface-2 rounded-lg">
                    <TrendingUp className="w-5 h-5 text-blue-500 flex-shrink-0 mt-0.5" />
                    <div>
                        <div className="text-sm font-bold text-pr-text-1">Activation Credits</div>
                        <div className="text-xs text-pr-text-2">Earn credits backed by brand budgets</div>
                    </div>
                </div>
            </div>
            ...
            >
                Connect Instagram & Start Activating
                <ArrowRight className="w-5 h-5" />
            </button>

            <p className="text-center text-xs text-pr-text-muted mt-4">
                Verified brand-funded reward model. No speculation. Real results.
            </p>
        </div>
    );
}
